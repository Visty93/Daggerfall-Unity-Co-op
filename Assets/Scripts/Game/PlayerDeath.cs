...
namespace DaggerfallWorkshop.Game
{
    ...
    public class PlayerDeath : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            EnsureMultiplayerRespawnManager();
        }
        ...
        void Update()
        {
            if (deathInProgress && mainCamera)
            {
                ...
                if (IsMultiplayerRespawnActive())
                {
                    MultiplayerRespawnManager respawnManager = GetMultiplayerRespawnManager();

                    // In MP, never let the vanilla death flow go to the title menu.
                    // Default MP respawn intentionally waits longer than vanilla death, so
                    ...
                    {
                        if (respawnManager != null)
                            respawnManager.RequestRespawnNow("death-timeout");
                        return;
                    }
                }
                else if (Time.fixedTime - timeOfDeath > TimeBeforeReset)
                {
        ...
        /// <summary>
        /// True when this death should be handled by the MP respawn path instead of vanilla title/menu death.
        /// </summary>
        public bool IsMultiplayerRespawnActive()
        {
            return MultiplayerRespawnManager.IsMultiplayerActive();
        }
        ...
        private void EnsureMultiplayerRespawnManager()
        {
            if (GetComponent<MultiplayerRespawnManager>() == null)
                gameObject.AddComponent<MultiplayerRespawnManager>();
        }

        private MultiplayerRespawnManager GetMultiplayerRespawnManager()
        {
            MultiplayerRespawnManager respawnManager = GetComponent<MultiplayerRespawnManager>();
            if (respawnManager == null)
                respawnManager = gameObject.AddComponent<MultiplayerRespawnManager>();
        ...
        private void PlayerEntity_OnDeath(DaggerfallEntity entity)
        {
            ...
            timeOfDeathRealtime = Time.realtimeSinceStartup;
            ...
            if (IsMultiplayerRespawnActive())
            {
                MultiplayerRespawnManager respawnManager = GetMultiplayerRespawnManager();
                if (respawnManager != null)
                    respawnManager.NotifyLocalDeathStarted("player-death");
            }
