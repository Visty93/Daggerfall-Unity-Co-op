...
namespace DaggerfallWorkshop.Game
{
    ...
    public class WeaponManager : MonoBehaviour
    {
            ...
            else if (!isDamageFinished && ScreenWeapon.GetCurrentFrame() == ScreenWeapon.GetHitFrame())
            {
                ...
                else
                {
                    ...
                    if (missile)
                    {
                        ...
                        // The owning player already has the real damaging arrow. Report a
                        // separate cosmetic shot so other clients can create a harmless
                        // local arrow visual from this player's multiplayer shell.
                        ReportPlayerArrowShotVisual();
                    }
                }
        ...
        public bool WeaponDamage(DaggerfallUnityItem strikingWeapon, bool arrowHit, bool arrowSummoned, Transform hitTransform, Vector3 impactPosition, Vector3 direction)
        {
            ...
            if (mobileNpc)
            {
                bool syncedRemovalHandled = false;

                if (!mobileNpc.IsGuard)
                {
                    // Report the exact soft-synced NPC before WeaponManager disables it.
                    // Otherwise MobileNpcSync sees only an inactive object and can discard the
                    // owner record before a death packet reaches the other clients.
                    syncedRemovalHandled = MobileNpcSync.TryReportWeaponMobileNpcRemoval(
                        mobileNpc,
                        impactPosition,
                        true,
                        true);

                    if (!syncedRemovalHandled)
                        MobileNpcSync.ShowMobileNpcBlood(mobileNpc, impactPosition);
                ...
                else
                {
                    ...
                    // On a pure client SpawnCityGuard() relays the request to the server and
                    // intentionally returns null. Remove the soft-synced guard shell everywhere,
                    // play blood, and do not dereference that null local guard.
                    syncedRemovalHandled = MobileNpcSync.TryReportWeaponMobileNpcRemoval(
                        mobileNpc,
                        impactPosition,
                        false,
                ...
                if (!syncedRemovalHandled && mobileNpc.Motor)
                    mobileNpc.Motor.gameObject.SetActive(false);
            }
            ...
            if (entityBehaviour)
            {
                if (entityBehaviour.EntityType == EntityTypes.EnemyMonster || entityBehaviour.EntityType == EntityTypes.EnemyClass)
                {
                    ...
                    if (damage > 0)
                    {
                        ...
                        bool playPainVoice = DaggerfallUnity.Settings.CombatVoices &&
                            entityBehaviour.EntityType == EntityTypes.EnemyClass &&
                        ...
                        ReportNetworkedEnemyHitCosmetics(hitTransform, impactPosition, strikingWeapon != null, playPainVoice, heavyDamage);
                    }
                    else
                    {
        ...
        private void ReportPlayerArrowShotVisual()
        {
            ...
            PlayerMultiplayer localPlayer = PlayerMultiplayer.GetLocalPlayerForCommand("player-arrow-shot-visual");
            if (localPlayer == null)
                localPlayer = PlayerMultiplayer.GetAnyPlayerForCommandFallback("player-arrow-shot-visual");
            if (localPlayer == null)
                return;

            localPlayer.CmdReportPlayerArrowShotVisual(aimDirection.normalized);
        }

        private void ReportNetworkedEnemyHitCosmetics(Transform hitTransform, Vector3 impactPosition, bool weaponHit, bool playPainVoice, bool heavyDamage)
        {
            if (!NetworkClient.active || hitTransform == null)
                return;

            NetworkIdentity targetIdentity = hitTransform.GetComponent<NetworkIdentity>();
            if (targetIdentity == null)
                return;

            PlayerMultiplayer localPlayer = PlayerMultiplayer.GetLocalPlayerForCommand("player-hit-enemy-cosmetics");
            if (localPlayer == null)
                localPlayer = PlayerMultiplayer.GetAnyPlayerForCommandFallback("player-hit-enemy-cosmetics");
            if (localPlayer == null)
                return;

            localPlayer.CmdReportPlayerHitEnemyCosmetics(targetIdentity.netId, impactPosition, weaponHit, playPainVoice, heavyDamage);
        }
