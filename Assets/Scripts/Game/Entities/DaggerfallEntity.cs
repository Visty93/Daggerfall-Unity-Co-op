...
namespace DaggerfallWorkshop.Game.Entity
{
    ...
    public abstract class DaggerfallEntity
    {
        ...
        public int MaxHealth
        {
            ...
            {
                int current, max;
                if (TryGetMultiplayerShellHealth(out current, out max))
                    return max;
        ...
        public float CurrentHealthPercent
        {
            get
            {
                ...
                return Mathf.Clamp01(CurrentHealth / (float)max);
            }
        }
...
public virtual int SetHealth(int amount, bool restoreMode = false)
{
    ...
    if (currentHealth <= 0)
    {
        ...
        {
            // Call CheckMulti() in the DaggerfallEntityBehaviour to handle multiplayer-specific death logic
            entityBehaviour.CheckMulti();
        }
    }
    ...
    return currentHealth;
        ...
        // Compatibility path for unmodified healthbar mods.
        // Remote PlayerMultiplayer visuals are fake enemy shells, so mods often read
        // their DaggerfallEntityBehaviour.Entity health. When this entity lives under
        // a PlayerMultiplayer object, return the synced real player health instead of
        // the generic fake enemy health.
        bool TryGetMultiplayerShellHealth(out int current, out int max)
        {
            current = 0;
            max = 0;
            ...
            // Never redirect the actual PlayerEntity. Only fake shell entities under
            // PlayerMultiplayer should use this compatibility path.
            if (this is PlayerEntity)
                return false;

            global::PlayerMultiplayer pm = entityBehaviour.GetComponentInParent<global::PlayerMultiplayer>();
            if (pm == null || pm.PlayerMPMaxHealth <= 0)
                return false;

            max = Mathf.Max(1, pm.PlayerMPMaxHealth);
            current = Mathf.Clamp(pm.PlayerMPCurrentHealth, 0, max);
            return true;
        }
