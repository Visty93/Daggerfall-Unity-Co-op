...
public void SetMobileEnemy(MobileEnemy newMobileEnemy)
{
    this.mobileEnemy = newMobileEnemy;
}
...
public override void Update(DaggerfallEntityBehaviour sender)
{
        ...
        !GameManager.Instance.PlayerEffectManager.IsTransformedLycanthrope())
    {
        ...
        var ni = sender.GetComponent<Mirror.NetworkIdentity>();
        bool isNetworked = mpActive && ni != null;

        // 1) MP: clients must NOT decide despawn (prevents "invisible guards" desync).
        if (isNetworked && Mirror.NetworkClient.active && !Mirror.NetworkServer.active)
            return;

        // 2) Server: do not auto-despawn guards that were spawned for a specific requester.
        //    Otherwise host crime=none would delete client-requested guards.
        if (Mirror.NetworkServer.active)
        {
            var ewp = sender.GetComponent<EnemyWorldPosition>();
            if (ewp != null && ewp.requesterNetId != 0)
                return;
        }

        // 3) Vanilla rule (SP or server-owned vanilla guards): despawn when crime clears.
        if (GameManager.Instance.PlayerEntity.CrimeCommitted == PlayerEntity.Crimes.None)
        {
            if (Mirror.NetworkServer.active && isNetworked)
                Mirror.NetworkServer.Destroy(sender.gameObject);
            else
        ...
        /// </summary>
        public void SetEnemyCareer(MobileEnemy mobileEnemy, EntityTypes entityType, int levelOverride = 0)
        {
            int effectivePlayerLevel = levelOverride > 0
                ? Mathf.Clamp(levelOverride, 1, 100)
                : ((GameManager.Instance != null && GameManager.Instance.PlayerEntity != null) ? Mathf.Clamp(GameManager.Instance.PlayerEntity.Level, 1, 100) : 1);
