...
namespace DaggerfallWorkshop.Game.Entity
{
    ...
    public class PlayerEntity : DaggerfallEntity
    {
        ...
        public override void Update(DaggerfallEntityBehaviour sender)
        {
            ...
            {
                // Multiplayer time sync can legitimately move the local clock backwards
                // when the host is authoritative. The old single-player safety exception
                // leaves PlayerEntity stuck with a future lastGameMinutes value, which
                // blocks minute-based logic such as intermittent enemy spawning until
                // the host naturally catches up.
                if (Mirror.NetworkClient.active || Mirror.NetworkServer.active)
                {
                    Debug.LogWarning(string.Format("[TimeSync] Correcting PlayerEntity.LastGameMinutes after network time rollback. lastGameMinutes={0}, gameMinutes={1}", lastGameMinutes, gameMinutes));
            ...
            if (!GameManager.Instance.PlayerEnterExit.IsPlayerInside)
            {
                ...
                if (GameManager.Instance.PlayerGPS.IsPlayerInLocationRect)
                {
                    if (timeOfDay < 360 || timeOfDay > 1080)
                    {
                        ...
                        {
                            GameObjectHelper.CreateFoeSpawner(true, RandomEncounters.ChooseRandomEnemy(false, GameManager.Instance.PlayerEntity.Level), 1, minLocationDistance);
                            return true;
                        }
                    }
                }
                else
                {
                    ...
                    GameObjectHelper.CreateFoeSpawner(true, RandomEncounters.ChooseRandomEnemy(false, GameManager.Instance.PlayerEntity.Level), 1, minWildernessDistance);
                    return true;
                }
            }
                ...
                if (GameManager.Instance.PlayerEnterExit.IsPlayerInsideDungeon)
                {
                    if (isResting)
                    {
                        if (FormulaHelper.RollRandomSpawn_Dungeon() == 0)
                        {
                            ...
                            GameObjectHelper.CreateFoeSpawner(false, RandomEncounters.ChooseRandomEnemy(false, GameManager.Instance.PlayerEntity.Level), 1, minDungeonDistance);
                            return true;
                        }
                    }
                }
       ...
       // Multiplayer guard-spawn safety helpers.
        // Pure clients already relay guard requests through PlayerMultiplayer commands.
        // Hosts are both client and server, so they used to skip that throttled path and
        // could spawn several exterior guard waves if a quest/crime event re-entered this
        // method repeatedly in the same second.
        private bool IsMultiplayerGuardSpawningActive()
        {
            return Mirror.NetworkClient.active || Mirror.NetworkServer.active;
        }

        private int GetActiveCityWatchGuardCount()
        {
            return GameManager.Instance.HowManyEnemiesOfType(MobileTypes.Knight_CityWatch, false, true);
        }

        private int GetMultiplayerGuardSpawnBudget()
        {
            return Mathf.Max(0, PlayerMultiplayer.SERVER_MAX_GUARDS_PER_REQUESTER - GetActiveCityWatchGuardCount());
        }

        private void SetMultiplayerGuardRequestCooldown()
        {
            if (IsMultiplayerGuardSpawningActive())
                nextGuardRequestTime = Time.unscaledTime + PlayerMultiplayer.SERVER_GUARD_COOLDOWN_SECONDS;
        }

        private uint GetLocalMultiplayerGuardRequesterNetId()
        {
            if (!Mirror.NetworkServer.active)
                return 0U;

            PlayerMultiplayer pm = PlayerMultiplayer.GetLocalPlayer();
            if (pm != null)
                return pm.netId;

            if (Mirror.NetworkServer.localConnection != null && Mirror.NetworkServer.localConnection.identity != null)
                return Mirror.NetworkServer.localConnection.identity.netId;
...
// ================================================================
// Recreation of guard spawning based on classic (network-aware)
// - CLIENT: compute exact positions like SP/host, then ask server
// - SERVER/SP: run original logic locally
// ================================================================
public void SpawnCityGuards(bool immediateSpawn)
{
    const int maxActiveGuardSpawns = 5;
    bool multiplayerGuardSpawningActive = IsMultiplayerGuardSpawningActive();

    // Shared MP throttle. Pure clients and hosts both pass through here, so a rare
    // per-frame quest/crime re-entry cannot create a new guard wave every frame.
    if (multiplayerGuardSpawningActive && Time.unscaledTime < nextGuardRequestTime)
        return;

    // ---- CLIENT PATH: compute positions & ask server; never spawn locally ----
    if (Mirror.NetworkClient.active && !Mirror.NetworkServer.active)
    {
        // throttle so we don't spam the server if caller loops this
        if (Time.unscaledTime < nextGuardRequestTime)
            return;

        PlayerMultiplayer pm = PlayerMultiplayer.localPlayer ?? UnityEngine.Object.FindObjectOfType<PlayerMultiplayer>();
        if (pm != null)
        {
            var enterExit = GameManager.Instance.PlayerEnterExit;

            // A pure client must never turn a dungeon guard event into a network spawn request.
            // Clear any delayed town guard arrival left over from before the transition as well.
            if (enterExit == null || enterExit.IsPlayerInsideDungeon)
            {
                guardsArriveCountdown = 0f;
                guardsArriveCountdownLocation = null;
                return;
            }

            bool isInteriorAtRequest = enterExit.IsPlayerInsideBuilding;

            // We’ll collect exact positions+forwards like SP would have used.
            var posList = new System.Collections.Generic.List<Vector3>(6);
            var fwdList = new System.Collections.Generic.List<Vector3>(6);

            // Only spawn if player is not in a dungeon
            if (!enterExit.IsPlayerInsideDungeon)
            {
                // Interior handling (open shop / tavern / residence)
                if (enterExit.IsPlayerInside &&
                    (enterExit.IsPlayerInsideOpenShop || enterExit.IsPlayerInsideTavern || enterExit.IsPlayerInsideResidence))
                {
                    if (enterExit.Interior != null &&
                        enterExit.Interior.FindLowestOuterInteriorDoor(out Vector3 lowestDoorPos, out Vector3 lowestDoorNormal))
                    {
                        // Classic offsets the position slightly along door normal
                        ...
                        {
                            posList.Add(lowestDoorPos);
                            // Classic uses Vector3.forward as facing for interior spawn
                            fwdList.Add(Vector3.forward);
                        }

                        pm.CmdRequestGuardSpawnAtPositions(posList.ToArray(), fwdList.ToArray(), isInteriorAtRequest);
                        SetMultiplayerGuardRequestCooldown();
                        return;
                    }
                }

                // We are in a town exterior (or interior without a valid door): use population NPCs like SP
                ...
                if (dfLocation != null)
                {
                    ...
                    if (populationManager != null)
                    {
                        if (immediateSpawn)
                        {
                            ...
                            {
                                var pop = populationManager.PopulationPool[i];
                                if (!pop.npc || !pop.npc.isActiveAndEnabled) continue;

                                Vector3 directionToMobile = pop.npc.Motor.transform.position - GameManager.Instance.PlayerMotor.transform.position;
                                if (directionToMobile.magnitude <= 77.5f)
                                {
                                    // Guard NPCs first
                                    if (pop.npc.IsGuard)
                                    {
                                        posList.Add(pop.npc.transform.position);
                                        fwdList.Add(pop.npc.transform.forward);
                                        guardsSpawnedFromNPCs++;
                                        pop.npc.gameObject.SetActive(false);
                                    }
                                    // Non-guards with angle & 25% chance (classic)
                                    else if (Vector3.Angle(directionToMobile, GameManager.Instance.PlayerMotor.transform.forward) >= 105.469f
                            ...
                            // If we didn't find any NPC to spawn from, emulate classic's fallback spawner:
                            if (guardsSpawnedFromNPCs == 0)
                            {
                                // Classic: foe spawner with 2..5 guards around the player.
                                int count = UnityEngine.Random.Range(2, 5 + 1);
                                BuildGuardRingAroundPlayer(GameManager.Instance.PlayerMotor.transform, count, out var ringPos, out var ringFwd);
                                posList.AddRange(ringPos);
                                fwdList.AddRange(ringFwd);
                            }
                        ...
                        {
                            // Non-immediate: only if seen by NPC (classic)
                            bool seen = false;
                            ...
                            for (int i = 0; i < populationManager.PopulationPool.Count; i++)
                            {
                                ...
                                Vector3 toPlayer = GameManager.Instance.PlayerMotor.transform.position - pop.npc.Motor.transform.position;
                                if (toPlayer.magnitude <= 77.5f && Vector3.Angle(toPlayer, pop.npc.Motor.transform.forward) <= 95f)
                                {
                                    // LOS check
                                    Vector3 eyePos = pop.npc.Motor.transform.position; eyePos.y += 0.7f;
                                    CharacterController controller = GameManager.Instance.PlayerEntityBehaviour.transform.GetComponent<CharacterController>();
                                    Vector3 playerEyePos = GameManager.Instance.PlayerMotor.transform.position; playerEyePos.y += controller.height / 3f;

                                    if (Physics.Raycast(new Ray(eyePos, (playerEyePos - eyePos).normalized), out RaycastHit hit, 77.5f))
                                    {
                                        var ent = hit.transform.gameObject.GetComponent<DaggerfallEntityBehaviour>();
                                        if (ent == GameManager.Instance.PlayerEntityBehaviour)
                                            seen = true;
                                        if (pop.npc.IsGuard)
                                            seenByGuard = true;
                                    }
                                ...
                                if (seenByGuard)
                                {
                                    // Spawn 1 guard from this guard NPC, like classic
                                    posList.Add(pop.npc.transform.position);
                                    fwdList.Add(pop.npc.transform.forward);
                                    pop.npc.gameObject.SetActive(false);
                            ...
                            // Not seen by a guard but seen by a non-guard → start local countdown (classic)
                            if (!seenByGuard && seen)
                            {
            ...
            // No valid location/population path: do not request guards from a pure client.
        }

        SetMultiplayerGuardRequestCooldown(); // small client-side cooldown
        return;
    }
    ...
    // In multiplayer, use the same hard requester cap used by PlayerMultiplayer commands
    // instead of the old host-only pre-check that could overshoot by a full population scan.
    int activeCityWatchGuards = GetActiveCityWatchGuardCount();
    bool underGuardSpawnGate = multiplayerGuardSpawningActive
        ? activeCityWatchGuards < PlayerMultiplayer.SERVER_MAX_GUARDS_PER_REQUESTER
        : activeCityWatchGuards <= maxActiveGuardSpawns;

    if (!GameManager.Instance.PlayerEnterExit.IsPlayerInsideDungeon && underGuardSpawnGate)
    {
        if (multiplayerGuardSpawningActive)
            SetMultiplayerGuardRequestCooldown();
        ...
        if (enterExit.IsPlayerInside && (enterExit.IsPlayerInsideOpenShop || enterExit.IsPlayerInsideTavern || enterExit.IsPlayerInsideResidence))
        {
            if (enterExit.Interior.FindLowestOuterInteriorDoor(out Vector3 lowestDoorPos, out Vector3 lowestDoorNormal))
            {
                ...
                int guardCount = UnityEngine.Random.Range(2, 6);
                if (multiplayerGuardSpawningActive)
                    guardCount = Mathf.Min(guardCount, GetMultiplayerGuardSpawnBudget());
        ...
        {
            int guardsSpawnedFromNPCs = 0;
            int guardSpawnBudget = multiplayerGuardSpawningActive ? GetMultiplayerGuardSpawnBudget() : int.MaxValue;
            if (guardSpawnBudget <= 0)
                return;
            ...
            for (int i = 0; i < populationManager.PopulationPool.Count; i++)
            {
                ...
                if (directionToMobile.magnitude <= 77.5f)
                {
                    if (populationManager.PopulationPool[i].npc.IsGuard)
                    {
                        ...
                        if (multiplayerGuardSpawningActive && guardsSpawnedFromNPCs >= guardSpawnBudget)
                            break;
                    }
                    else if (Vector3.Angle(directionToMobile, GameManager.Instance.PlayerMotor.transform.forward) >= 105.469f
                             && UnityEngine.Random.Range(0, 4) == 0)
            ...
            if (guardsSpawnedFromNPCs == 0)
            {
                if (multiplayerGuardSpawningActive)
                {
                    int count = Mathf.Min(UnityEngine.Random.Range(2, 5 + 1), GetMultiplayerGuardSpawnBudget());
                    if (count > 0)
                    {
                        BuildGuardRingAroundPlayer(GameManager.Instance.PlayerMotor.transform, count, out Vector3[] ringPos, out Vector3[] ringFwd);
                        for (int i = 0; i < count; i++)
                            SpawnCityGuard(ringPos[i], ringFwd[i]);
                    }
                }
                else
                {
        ...
        else
        {
            ...
            for (int i = 0; i < populationManager.PopulationPool.Count; i++)
            {
                ...
                if (toPlayer.magnitude <= 77.5f && Vector3.Angle(toPlayer, populationManager.PopulationPool[i].npc.Motor.transform.forward) <= 95f)
                {
                    // LOS check
                    Vector3 eyePos = populationManager.PopulationPool[i].npc.Motor.transform.position; eyePos.y += .7f;
                    CharacterController controller = GameManager.Instance.PlayerEntityBehaviour.transform.GetComponent<CharacterController>();
                    Vector3 playerEyePos = GameManager.Instance.PlayerMotor.transform.position; playerEyePos.y += controller.height / 3f;
                ...
                if (seenByGuard)
                {
                    if (!multiplayerGuardSpawningActive || GetMultiplayerGuardSpawnBudget() > 0)
                    {
                        SpawnCityGuard(populationManager.PopulationPool[i].npc.transform.position, populationManager.PopulationPool[i].npc.transform.forward);
                        populationManager.PopulationPool[i].npc.gameObject.SetActive(false);
                    }
...
// Single-guard spawn. On client this just relays to server, on host/SP it spawns locally.
public GameObject SpawnCityGuard(Vector3 position, Vector3 direction)
{
    if (Mirror.NetworkClient.active && !Mirror.NetworkServer.active)
    {
        ...
        if (pm != null)
        {
            bool isInterior = GameManager.Instance.PlayerEnterExit.IsPlayerInsideBuilding;
            pm.CmdSpawnCityGuard(position, direction, isInterior);
        }
        return null; // client never spawns locally
    }

    // Host / SP path (unchanged but null-safe)
    ...
    // Stamp host-spawned MP guards with the host requester netId too. Without this,
    // culling sees them as requester 0/no-owner guards and the per-requester guard cap
    // does not reliably apply to host-originated city watch.
    if (Mirror.NetworkServer.active)
    {
        EnemyWorldPosition ewp = cityWatch[0].GetComponent<EnemyWorldPosition>();
        if (ewp != null)
        {
            bool isInteriorAtRequest = GameManager.Instance.PlayerEnterExit != null && GameManager.Instance.PlayerEnterExit.IsPlayerInsideBuilding;
            ewp.SetSpawnContext(isInteriorAtRequest, GetLocalMultiplayerGuardRequesterNetId());
        }
    }
...
// Classic compatibility helper for client ring fallback
private static void BuildGuardRingAroundPlayer(Transform player, int count, out Vector3[] positions, out Vector3[] forwards)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        positions[i] = p;
        forwards[i]  = (center - p).normalized; // face towards player
    }
}
        ...
        {
            int activeCityWatchGuards = GameManager.Instance.HowManyEnemiesOfType(MobileTypes.Knight_CityWatch, true);
            if (activeCityWatchGuards <= 0)
                return;

            bool multiplayerGuardSpawningActive = IsMultiplayerGuardSpawningActive();
            int guardSpawnBudget = multiplayerGuardSpawningActive
                ? Mathf.Max(0, PlayerMultiplayer.SERVER_MAX_GUARDS_PER_REQUESTER - activeCityWatchGuards)
                : int.MaxValue;
            ...
            for (int i = 0; i < populationManager.PopulationPool.Count; i++)
            {
                ...
                // Spawn from guard mobile NPCs, but in MP do not let the follow-up
                // conversion path exceed the same active city-watch cap.
                if (populationManager.PopulationPool[i].npc.IsGuard)
                {
                    SpawnCityGuard(populationManager.PopulationPool[i].npc.transform.position, populationManager.PopulationPool[i].npc.transform.forward);
                    populationManager.PopulationPool[i].npc.gameObject.SetActive(false);
