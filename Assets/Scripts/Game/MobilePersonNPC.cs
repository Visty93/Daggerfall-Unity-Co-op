...
namespace DaggerfallWorkshop.Game
{
    ...
    public class MobilePersonNPC : MonoBehaviour
    {
        ...
        /// <summary>
        /// Applies deterministic/synced person data without rolling any random values.
        /// Used by MobileNpcSync so remote ghost civilians have the exact same visible identity.
        /// </summary>
        public void ApplySyncedPerson(Races race, Genders gender, int outfitVariant, bool isGuard, int personFaceVariant, int personFaceRecordId, string npcName)
        {
            if (outfitVariant < 0)
                outfitVariant = 0;
            if (outfitVariant >= numPersonOutfitVariants)
                outfitVariant = numPersonOutfitVariants - 1;
            if (personFaceVariant < 0)
                personFaceVariant = 0;
            if (personFaceVariant >= numPersonFaceVariants)
                personFaceVariant = numPersonFaceVariants - 1;

            this.gender = gender;
            this.personOutfitVariant = outfitVariant;
            this.IsGuard = isGuard;
            this.personFaceRecordId = personFaceRecordId;
            this.nameNPC = string.IsNullOrEmpty(npcName) ? "Civilian" : npcName;

            SetRace(race);

            if (Asset)
                Asset.SetPerson(this.race, this.gender, this.personOutfitVariant, this.IsGuard, personFaceVariant, this.personFaceRecordId);
        }

        /// <summary>
        /// Reconstructs the random face variant from the stored face record id.
        /// </summary>
        public int GetPersonFaceVariant()
        {
            int baseRecord = GetFaceRecordBaseIndex(race, gender, personOutfitVariant);
            int variant = personFaceRecordId - baseRecord;
            return Mathf.Clamp(variant, 0, numPersonFaceVariants - 1);
        }
        ...
        int GetFaceRecordBaseIndex(Races race, Genders gender, int outfitVariant)
        {
            if (outfitVariant < 0)
                outfitVariant = 0;
            if (outfitVariant >= numPersonOutfitVariants)
            ...
            return recordIndices[outfitVariant];
        }
