...
namespace DaggerfallWorkshop.Game.Formulas
{
    ...
    public static class FormulaHelper
    {
        ...
        private static bool IsLocalPlayerMonsterHitTarget(DaggerfallEntity target)
        {
            return target != null && GameManager.Instance != null && target == GameManager.Instance.PlayerEntity;
        }

        private static bool IsPlayerMonsterHitTarget(DaggerfallEntity target)
        {
            return target != null && target.EntityBehaviour != null && target.EntityBehaviour.EntityType == EntityTypes.Player;
        }

        private static string MonsterHitName(EnemyEntity attacker)
        {
            return attacker != null && attacker.EntityBehaviour != null ? attacker.EntityBehaviour.name : "null";
        }

        private static uint GetMonsterHitAttackerNetId(EnemyEntity attacker)
        {
            if (attacker == null || attacker.EntityBehaviour == null)
                return 0;

            NetworkIdentity identity = attacker.EntityBehaviour.GetComponent<NetworkIdentity>();
            return identity != null ? identity.netId : 0;
        }

        private static void ShowLocalMonsterHitSpecialSparkles(EnemyEntity attacker)
        {
            if (attacker == null || attacker.EntityBehaviour == null)
                return;

            EnemyBlood sparkles = attacker.EntityBehaviour.GetComponent<EnemyBlood>();
            CharacterController controller = attacker.EntityBehaviour.GetComponent<CharacterController>();
            if (sparkles == null || controller == null)
                return;

            Vector3 sparklesPos = attacker.EntityBehaviour.transform.position + controller.center;
            sparklesPos.y += controller.height / 8f;
            sparkles.ShowMagicSparkles(sparklesPos);
        }

        private static void ReportLocalMonsterHitSpecialCosmetics(EnemyEntity attacker, string reason)
        {
            if (!NetworkClient.active || attacker == null || attacker.EntityBehaviour == null)
                return;

            uint enemyNetId = GetMonsterHitAttackerNetId(attacker);
            if (enemyNetId == 0)
                return;

            global::PlayerMultiplayer localPlayer = global::PlayerMultiplayer.GetLocalPlayerForCommand("monster-hit-special-" + reason);
            if (localPlayer == null)
                localPlayer = global::PlayerMultiplayer.GetAnyPlayerForCommandFallback("monster-hit-special-" + reason);

            if (localPlayer != null)
            {
                Debug.Log($"[MPMonsterHitSpecial][CosmeticsReport] reason={reason} attacker='{MonsterHitName(attacker)}' enemyNetId={enemyNetId}");
                localPlayer.CmdReportLocalPlayerSpellEffectCosmetics(enemyNetId);
            }
        }

        private static void NotifyLocalMonsterHitSpecialResolved(EnemyEntity attacker, string reason)
        {
            // The actual gameplay effect is local to PlayerAdvanced. This is only the
            // generic visible feedback path for host/other clients watching the remote
            // PlayerMultiplayer shell.
            if (IsNetworkActive() && IsLocalPlayerMonsterHitTarget(GameManager.Instance != null ? GameManager.Instance.PlayerEntity : null))
            {
                ShowLocalMonsterHitSpecialSparkles(attacker);
                ReportLocalMonsterHitSpecialCosmetics(attacker, reason);
            }
        }

        private static bool TryAssignMonsterHitParalyzeDirectlyToLocalPlayer(EnemyEntity attacker, DaggerfallEntity target)
        {
            if (!IsNetworkActive() || !IsLocalPlayerMonsterHitTarget(target))
                return false;

            EntityEffectManager targetEffectManager = target != null && target.EntityBehaviour != null
                ? target.EntityBehaviour.GetComponent<EntityEffectManager>()
                : null;

            if (targetEffectManager == null)
            {
                Debug.LogWarning($"[MPMonsterHitSpecial][NoLocalEffectManager] reason=paralyze attacker='{MonsterHitName(attacker)}'.");
                return true;
            }

            if (targetEffectManager.FindIncumbentEffect<Paralyze>() != null)
            {
                Debug.Log($"[MPMonsterHitSpecial][SkipLocalParalyze] attacker='{MonsterHitName(attacker)}' alreadyParalyzed=True");
                return true;
            }
            ...
            GameManager.Instance.EntityEffectBroker.ClassicSpellRecordDataToEffectBundleSettings(spellData, BundleTypes.Spell, out bundleSettings);

            EntityEffectBundle spell = new EntityEffectBundle(bundleSettings, attacker != null ? attacker.EntityBehaviour : null);

            Debug.Log($"[MPMonsterHitSpecial][AssignLocalPlayerSpecial] reason=paralyze attacker='{MonsterHitName(attacker)}' spellId=66 effects={(bundleSettings.Effects != null ? bundleSettings.Effects.Length : -1)}");
            targetEffectManager.AssignBundle(spell, AssignBundleFlags.ShowNonPlayerFailures);
            ShowLocalMonsterHitSpecialSparkles(attacker);
            ReportLocalMonsterHitSpecialCosmetics(attacker, "paralyze");
            return true;
        }

        /// <summary>
        /// Execute special monster attack effects.
        /// In multiplayer, player-affecting monster-hit specials are resolved on the
        /// machine that owns the real local PlayerAdvanced. This keeps the actual disease,
        /// paralysis, vampirism, fatigue drain, saving throw, and immunity logic local to
        /// the affected player, while only a generic cosmetic sparkle is reported outward.
        /// </summary>
        /// <param name="attacker">Attacking entity</param>
        /// <param name="target">Target entity</param>
        ...
        public static void OnMonsterHit(EnemyEntity attacker, DaggerfallEntity target, int damage)
        {
            ...
            if (attacker == null || target == null || target.EntityBehaviour == null)
                return;

            bool localPlayerTarget = IsNetworkActive() && IsLocalPlayerMonsterHitTarget(target);
            ...
            switch (attacker.CareerIndex)
            {
                case (int)MonsterCareers.Rat:
                    ...
                    if (Dice100.SuccessRoll(5))
                    {
                        ...
                        if (localPlayerTarget)
                            NotifyLocalMonsterHitSpecialResolved(attacker, "disease-rat");
                    }
                    break;
                case (int)MonsterCareers.GiantBat:
                    ...
                    if (Dice100.SuccessRoll(2))
                    {
                        InflictDisease(attacker, target, diseaseListB);
                        if (localPlayerTarget)
                            NotifyLocalMonsterHitSpecialResolved(attacker, "disease-giantbat");
                    }
                    break;
                case (int)MonsterCareers.Spider:
                case (int)MonsterCareers.GiantScorpion:
                    // Spider/scorpion paralysis is not in the normal enemy spell list. Classic
                    // spell 66 is created here after a successful monster hit. In MP, assigning
                    // it through attacker.readySpell fails on non-server clients because enemy
                    // magic real-cast is server gated. Apply directly to local PlayerAdvanced.
                    if (TryAssignMonsterHitParalyzeDirectlyToLocalPlayer(attacker, target))
                        break;

                    EntityEffectManager targetEffectManager = target.EntityBehaviour.GetComponent<EntityEffectManager>();
                    if (targetEffectManager == null)
                    {
                        Debug.LogWarning($"[MPMonsterHitSpecial][NoTargetEffectManager] reason=paralyze attacker='{MonsterHitName(attacker)}' target='{target.EntityBehaviour.name}'.");
                        break;
                    }
                    ...
                    if (targetEffectManager.FindIncumbentEffect<Paralyze>() == null)
                    {
                        ...
                        if (attackerEffectManager != null)
                            attackerEffectManager.SetReadySpell(spell, true);
                    }
                    break;
                case (int)MonsterCareers.Werewolf:
                    random = UnityEngine.Random.Range(0f, 100f);
                    if (random <= specialInfectionChance && IsPlayerMonsterHitTarget(target))
                    {
                        // Werewolf
                        ...
                        if (localPlayerTarget)
                            NotifyLocalMonsterHitSpecialResolved(attacker, "lycanthropy-werewolf");
                    }
                    break;
                case (int)MonsterCareers.Nymph:
                    FatigueDamage(attacker, target, damage);
                    if (localPlayerTarget)
                        NotifyLocalMonsterHitSpecialResolved(attacker, "fatigue-nymph");
                    break;
                case (int)MonsterCareers.Wereboar:
                    ...
                    if (random <= specialInfectionChance && IsPlayerMonsterHitTarget(target))
                    {
                        ...
                        if (localPlayerTarget)
                            NotifyLocalMonsterHitSpecialResolved(attacker, "lycanthropy-wereboar");
                    }
                    break;
                case (int)MonsterCareers.Zombie:
                    ...
                    if (Dice100.SuccessRoll(2))
                    {
                        ...
                        if (localPlayerTarget)
                            NotifyLocalMonsterHitSpecialResolved(attacker, "disease-zombie");
                    }
                    break;
                case (int)MonsterCareers.Mummy:
                    if (Dice100.SuccessRoll(5))
                    {
                        InflictDisease(attacker, target, diseaseListC);
                        if (localPlayerTarget)
                            NotifyLocalMonsterHitSpecialResolved(attacker, "disease-mummy");
                    }
                    break;
                ...
                case (int)MonsterCareers.VampireAncient:
                    ...
                    if (random <= specialInfectionChance && IsPlayerMonsterHitTarget(target))
                    {
                        ...
                        if (localPlayerTarget)
                            NotifyLocalMonsterHitSpecialResolved(attacker, "vampirism");
                    }
                    else if (random <= 2.0f)
                    {
                        InflictDisease(attacker, target, diseaseListA);
                        if (localPlayerTarget)
                            NotifyLocalMonsterHitSpecialResolved(attacker, "disease-vampire");
                    }
                    break;
                case (int)MonsterCareers.Lamia:
                    ...
                    FatigueDamage(attacker, target, damage);
                    if (localPlayerTarget)
                        NotifyLocalMonsterHitSpecialResolved(attacker, "fatigue-lamia");
                    break;
                default:
                    break;
            }
