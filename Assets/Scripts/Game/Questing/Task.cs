    ...
    public class Task
    {
        ...
        public void Update()
        {
            ...
            foreach (IQuestAction action in actions)
            {
                ...
                {
                    // Multiplayer nested quest authority must be runtime-only. A pure
                    // client pauses here until the authoritative child exists; unlike
                    // the old QuestNetSync suppression, this never calls SetComplete()
                    // just to block execution and therefore cannot poison a save.
                    // Returning also defers later EndQuest/reward actions in this task.
                    if (global::QuestNetSync.ShouldDeferNestedQuestStart(
                            ParentQuest,
                            this,
                            action))
                        return;

                    // The server can approve that this exact StartQuest was
                    // scheduled before the child enters QuestMachine. Skip only that
                    // action at runtime and continue later actions without saving the
                    // StartQuest as complete until the child genuinely exists.
                    if (global::QuestNetSync.ShouldSkipApprovedNestedQuestStart(
                            ParentQuest,
                            this,
                            action))
                        continue;
