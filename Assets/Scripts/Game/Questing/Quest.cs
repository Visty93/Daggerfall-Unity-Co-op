...
namespace DaggerfallWorkshop.Game.Questing
{
    ...
    public partial class Quest : IDisposable
    {
        ...
        // Multiplayer/echo safety: EndQuest() applies reputation and notebook side effects
        // immediately, while questComplete is only set a couple of ticks later.
        // Network replay can call EndQuest() again during that window, so guard those
        // one-time side effects independently from questComplete.
        bool endQuestEffectsApplied = false;
        ...
        {
            // If this quest is already complete/tombstoned, never apply end side effects again.
            if (questComplete || questTombstoned)
                return;
            ...
            // EndQuest() side effects must be one-shot. questComplete is delayed by ticksToEnd,
            // so a network echo can otherwise call this method again before questComplete=true
            // and give duplicate faction reputation/notebook entries.
            if (endQuestEffectsApplied)
                return;

            endQuestEffectsApplied = true;
        ...
        public DaggerfallMessageBox ShowMessagePopup(int id, bool immediate = false, bool oncePerQuest = false)
        {
            ...
            // Return the last chunk to close even for an immediate popup. Vanilla
            // execution is otherwise unchanged: QuestBreak is still raised and all
            // pending chunks are shown immediately. Multiplayer uses the returned box
            // only to attach an owner-close acknowledgement.
            DaggerfallMessageBox result =
                pendingMessageBoxStack.Peek();
        ...
        public struct QuestSaveData_v1
        {
            ...
            public bool questTombstoned;
            public DaggerfallDateTime questTombstoneTime;
            public bool endQuestEffectsApplied;
            public QuestSmallerDungeonsState smallerDungeonsState;
        ...
        public QuestSaveData_v1 GetSaveData()
        {
            ...
            data.endQuestEffectsApplied = endQuestEffectsApplied;
        ...
        public void RestoreSaveData(QuestSaveData_v1 data)
        {
            ...
            endQuestEffectsApplied = data.endQuestEffectsApplied;
            ...
            // QuestNetSync v15 and earlier permanently completed future StartQuest
            // actions on pure clients. Schedule a post-load, whole-collection repair.
            // This static path also runs in ordinary SP with no network player spawned.
            global::QuestNetSync.ScheduleLegacyQuestChainSaveRepair("quest-save-restore");
        }
