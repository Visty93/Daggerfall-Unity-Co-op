...
namespace DaggerfallWorkshop.Game.Questing
{
    ...
    public class QuestMachine : MonoBehaviour
    {
        ...
        public void Tick()
        {
            ...
            foreach (Quest quest in questsToInvoke)
            {
                if (quest != null)
                {
                    try
                    {
                        // StartQuest() already raises OnQuestStarted.
                        // Do not raise it a second time for scheduled quests.
                        StartQuest(quest);
                    }
                    catch (Exception ex)
                    {
        ...
        {
            // Keep vanilla singleplayer behaviour exactly unchanged.
            // The duplicate/quest-specific site-link handling is only needed when
            // Mirror is actually running as host or client.
            if (!IsMirrorMultiplayerActive())
            {
                siteLinks.Add(siteLink);
                return;
            }

            // Multiplayer can have several active quest instances targeting the same
            // building/dungeon at once. Avoid only exact duplicates for the same
            // quest/place, but allow different quest UIDs to link the same site.
            for (int i = 0; i < siteLinks.Count; i++)
            {
                SiteLink existing = siteLinks[i];
                if (existing.questUID == siteLink.questUID &&
                    existing.placeSymbol != null && siteLink.placeSymbol != null &&
                    existing.placeSymbol.Equals(siteLink.placeSymbol))
                    return;
            }
        ...
        /// <summary>
        /// Gets an active quest marker for a specific site type and map ID.
        /// This is used by callers that already know the logical location they want,
        /// rather than relying on the player's current PlayerGPS context.
        /// </summary>
        /// <param name="siteType">Type of site to search.</param>
        /// <param name="mapId">Map ID of the target location.</param>
        /// <param name="markerOut">Active QuestMarker out.</param>
        /// <returns>True if successful.</returns>
        public bool GetLocationQuestMarker(SiteTypes siteType, int mapId, out QuestMarker markerOut)
        {
            markerOut = new QuestMarker();

            SiteLink[] matchingSiteLinks = GetSiteLinks(siteType, mapId);
            if (matchingSiteLinks == null || matchingSiteLinks.Length == 0)
                return false;

            foreach (SiteLink link in matchingSiteLinks)
            {
                Quest quest = GetQuest(link.questUID);
                if (quest == null)
                    continue;
        ...
        public static bool HasSiteLink(Quest parentQuest, Symbol placeSymbol)
        {
            ...
            // Collect any SiteLinks associated with this site.
            ...
            // Keep vanilla singleplayer behaviour exactly unchanged: if any SiteLink
            // exists for this site, the place already has a link.
            if (!IsMirrorMultiplayerActive())
                return true;

            // Multiplayer-specific behaviour: another player's quest can target the
            // same building/dungeon. That must not suppress this quest's own SiteLink
            // or its placed resources can spawn from raw marker/local coordinates on
            // first entry. Only this same quest/place counts as an existing link.
            for (int i = 0; i < siteLinks.Length; i++)
            {
                SiteLink link = siteLinks[i];
                if (link.questUID == parentQuest.UID &&
                    link.placeSymbol != null && placeSymbol != null &&
                    link.placeSymbol.Equals(placeSymbol))
                    return true;
            }
        ...
        /// <summary>
        /// Returns true only while Mirror is actively running as host or client.
        /// This lets multiplayer quest/site-link fixes stay out of normal singleplayer.
        /// </summary>
        private static bool IsMirrorMultiplayerActive()
        {
