...
namespace DaggerfallWorkshop.Game.Questing
{
    ...
    public class Place : QuestResource
    {
        ...
        public void AssignQuestResource(Symbol targetSymbol, int markerIndex = -1, MarkerPreference markerIndexPreference = MarkerPreference.Default, bool cullExisting = true)
        {
            ...
            //
            // Multiplayer: one quest Foe resource can have several physical networked
            // instances at the current site (for example one host-created instance and
            // one client-requested instance). Remote/client-requested quest foes
            // intentionally do not occupy Foe.QuestResourceBehaviour on the listen-host,
            // so vanilla hot-remove below can only see one of those physical instances.
            // Before moving a Foe resource, let the server despawn every live physical
            // instance bound to this exact quest UID + Foe symbol. The resource itself is
            // not hidden or destroyed and can be assigned/spawned again at the new Place.
            // The MP physical-foe cull is only for a real relocation from ANOTHER
            // Place. Do not run it for a foe's first placement, or for another player's
            // replay of the same PlaceFoe at the same destination.
            //
            // Client-originated ClickedItem tasks can race like this:
            //   client PlaceFoe -> CmdSpawnQuestFoe -> server creates foe
            //   server replays same clicked task -> PlaceFoe
            // If we unconditionally cull here, that second execution destroys the foe
            // that was just requested by the client. When the host is not physically at
            // this Place it then cannot hot-spawn a replacement, producing the observed
            // split-second Daedra followed by no foe.
            bool foeAssignedAtDifferentPlace =
                resource is Foe &&
                IsQuestResourceAssignedToDifferentPlace(
                    resource.Symbol != null ? resource.Symbol : targetSymbol);

            bool multiplayerFoeRelocationManaged =
                cullExisting &&
                resource is Foe &&
                foeAssignedAtDifferentPlace &&
                (Mirror.NetworkServer.active || Mirror.NetworkClient.active);

            if (multiplayerFoeRelocationManaged && Mirror.NetworkServer.active)
            {
                MultiplayerCullPhysicalFoeInstancesForRelocation(
                    resource as Foe,
                    Symbol);
            }
            else if (cullExisting &&
                     resource is Foe &&
                     Mirror.NetworkServer.active)
            {
                Debug.Log(
                    $"[QuestFoeMP][RelocateCleanup] Kept physical foe(s) for first/same-place assignment " +
                    $"questUID={(ParentQuest != null ? ParentQuest.UID : 0UL)} " +
                    $"foe='{(resource.Symbol != null ? resource.Symbol.Name : targetSymbol.Name)}' " +
                    $"destination='{(Symbol != null ? Symbol.Name : string.Empty)}'");
            }
            ...
            // In MP, Foe relocation cleanup is server-authoritative above and removes
            // every matching networked physical instance. Do not let a pure client
            // locally Destroy() one Mirror replica, and do not repeat vanilla's
            // single-back-reference cleanup on the listen-host.
            if (!IsPlayerHere() && resource.QuestResourceBehaviour && !multiplayerFoeRelocationManaged)
            {
        ...
        /// <summary>
        /// Returns true only when this quest resource is currently assigned to a Place
        /// other than this destination Place. Used to distinguish real foe relocation
        /// from first placement / same-destination multiplayer replay.
        /// </summary>
        bool IsQuestResourceAssignedToDifferentPlace(Symbol targetSymbol)
        {
            if (ParentQuest == null || targetSymbol == null ||
                string.IsNullOrEmpty(targetSymbol.Name))
                return false;

            QuestResource[] places = ParentQuest.GetAllResources(typeof(Place));
            if (places == null)
                return false;

            for (int i = 0; i < places.Length; i++)
            {
                Place otherPlace = places[i] as Place;
                if (otherPlace == null || ReferenceEquals(otherPlace, this))
                    continue;

                if (MarkerContainsQuestResource(
                        otherPlace.siteDetails.selectedMarker,
                        targetSymbol))
                    return true;

                QuestMarker[] spawnMarkers =
                    otherPlace.siteDetails.questSpawnMarkers;
                if (spawnMarkers != null)
                {
                    for (int m = 0; m < spawnMarkers.Length; m++)
                    {
                        if (MarkerContainsQuestResource(
                                spawnMarkers[m],
                                targetSymbol))
                ...
                QuestMarker[] itemMarkers =
                    otherPlace.siteDetails.questItemMarkers;
                if (itemMarkers != null)
                {
                    for (int m = 0; m < itemMarkers.Length; m++)
                    {
                        if (MarkerContainsQuestResource(
                                itemMarkers[m],
                                targetSymbol))
        ...
        static bool MarkerContainsQuestResource(
            QuestMarker marker,
            Symbol targetSymbol)
        {
            ...
            {
                Symbol existing = marker.targetResources[i];
                if (existing != null &&
                    string.Equals(
                        existing.Name,
                        targetSymbol.Name,
        ...
        /// <summary>
        /// Multiplayer relocation cleanup for an already-instantiated quest Foe.
        /// Destroys all live physical instances linked to the same quest UID + Foe symbol
        /// on the server, including remote/client-requested instances that deliberately
        /// do not occupy the resource's single QuestResourceBehaviour back-reference.
        /// </summary>
        static void MultiplayerCullPhysicalFoeInstancesForRelocation(Foe foe, Symbol destinationPlace)
        {
            if (foe == null || foe.ParentQuest == null || foe.Symbol == null || !Mirror.NetworkServer.active)
                return;

            ulong questUID = foe.ParentQuest.UID;
            string foeSymbol = foe.Symbol.Name ?? string.Empty;
            if (questUID == 0UL || string.IsNullOrEmpty(foeSymbol))
                return;

            QuestResourceBehaviour[] behaviours = UnityEngine.Object.FindObjectsOfType<QuestResourceBehaviour>();
            if (behaviours == null || behaviours.Length == 0)
                return;

            int removed = 0;
            string destination = destinationPlace != null ? destinationPlace.Name : string.Empty;

            for (int i = 0; i < behaviours.Length; i++)
            {
                QuestResourceBehaviour qrb = behaviours[i];
                if (!qrb || qrb.QuestUID != questUID || qrb.TargetSymbol == null)
                    continue;

                if (!string.Equals(qrb.TargetSymbol.Name, foeSymbol, StringComparison.OrdinalIgnoreCase))
                    continue;

                // If this behaviour has already cached a resource, require it to be this
                // exact Foe object. The UID + symbol fallback still covers a just-created
                // QRB that has not reached Start()/CacheTarget() yet.
                if (qrb.TargetResource != null && !ReferenceEquals(qrb.TargetResource, foe))
                    continue;
                ...
                Mirror.NetworkIdentity identity = go.GetComponent<Mirror.NetworkIdentity>();
                uint netId = identity != null ? identity.netId : 0U;

                // Disable immediately so a foe scheduled for network destruction cannot
                // still attack, be clicked, or report another injured/killed event in the
                // remainder of this frame.
                if (go.activeSelf)
                    go.SetActive(false);

                if (identity != null && netId != 0U)
                ...
                {
                    Debug.Log(
                        $"[QuestFoeMP][RelocateCleanup] Despawned physical foe " +
                        $"questUID={questUID} symbol='{foeSymbol}' netId={netId} " +
                        $"destination='{destination}' go='{go.name}'");
                }
            }

            if (removed > 0 || Debug.isDebugBuild)
            {
                Debug.Log(
                    $"[QuestFoeMP][RelocateCleanup] Completed questUID={questUID} " +
                    $"symbol='{foeSymbol}' destination='{destination}' removed={removed}");
            }
        }
        ...
        void SetupLocalSite(string line)
        {
            ...
            if (!location.Loaded)
            {
                if (global::QuestNetSync.IsInRemoteParseWindow(ParentQuest.UID))
                {
                    if (Debug.isDebugBuild)
                        Debug.LogWarning($"[Place] SetupLocalSite: player not in a location for remote quest uid={ParentQuest.UID}. Deferring local-site resolution to QuestNetSync.");

                    siteDetails = new SiteDetails();
                    siteDetails.questUID = ParentQuest.UID;
                    siteDetails.siteType = SiteTypes.None;
                    siteDetails.regionName = string.Empty;
                    siteDetails.locationName = string.Empty;
                    siteDetails.buildingName = string.Empty;
                    siteDetails.mapId = 0;
                    siteDetails.locationId = 0;
                    siteDetails.regionIndex = 0;
                    siteDetails.buildingKey = 0;
                    siteDetails.questSpawnMarkers = null;
                    siteDetails.questItemMarkers = null;
                    return;
                }
...

            {
                ...
                {
                    if (Debug.isDebugBuild)
                        Debug.LogWarning($"[Place] SetupLocalSite: no local sites found for remote quest uid={ParentQuest.UID} symbol={Symbol?.Original} p2={p2} in {location.RegionName}/{location.Name}. Deferring to QuestNetSync.");
