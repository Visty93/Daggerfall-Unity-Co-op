...
namespace DaggerfallWorkshop.Game.Questing.Actions
{
    ...
    public class GetItem : ActionTemplate
    {
        ...
        public override void Update(Task caller)
        {
            ...
            // Multiplayer: only the quest taker should execute GetItem locally.
            try
            {
                if (!global::QuestNetSync.ShouldRunGetItem(ParentQuest.UID))
                {
                    if (Debug.isDebugBuild) Debug.Log($"[GetItem] Skipping GetItem on non-taker. uid={ParentQuest.UID}");
...
// Multiplayer: delivery/courier quests can contain multiple possible GetItem actions (book/weapon/etc).
// Only allow ONE of those random-delivery categories to execute per quest UID.
try
{
    if (!global::QuestNetSync.TryClaimRandomDeliveryGetItem(ParentQuest.UID, itemSymbol.Name))
    {
        if (Debug.isDebugBuild) Debug.Log($"[GetItem] Skipping extra random GetItem on taker. uid={ParentQuest.UID} symbol={itemSymbol.Name}");
        SetComplete();
        return;
    }
}
            ...
            // Multiplayer: replicate the granted quest item to other players.
            try
            {
                if (!item.DaggerfallUnityItem.IsOfTemplate(ItemGroups.Currency, (int)Currency.Gold_pieces))
                    global::QuestNetSync.ReportGetItemGranted(ParentQuest.UID, itemSymbol.Name, textId);
            }
            catch { }
