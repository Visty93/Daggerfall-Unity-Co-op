...
namespace DaggerfallWorkshop.Game.Questing.Actions
{
    ...
    public class PickOneOf : ActionTemplate
    {
        ...
        public override void Update(Task caller)
        {
            ...
            if(ParentQuest != null)
            {
                if (IsMultiplayerActive())
                    UnityEngine.Random.InitState(GetDeterministicMultiplayerSeed(caller));
                else
        ...
        {
            try { return Mirror.NetworkClient.active || Mirror.NetworkServer.active; }
            catch { return false; }
        }

        private int GetDeterministicMultiplayerSeed(Task caller)
        {
            unchecked
            {
                int hash = 17;
                hash = hash * 31 + ParentQuest.UID.GetHashCode();

                if (caller != null && caller.Symbol != null && !string.IsNullOrEmpty(caller.Symbol.Name))
                    hash = hash * 31 + StringComparer.OrdinalIgnoreCase.GetHashCode(caller.Symbol.Name);

                if (taskSymbols != null)
                {
                    for (int i = 0; i < taskSymbols.Length; i++)
                    {
                        if (taskSymbols[i] != null && !string.IsNullOrEmpty(taskSymbols[i].Name))
                            hash = hash * 31 + StringComparer.OrdinalIgnoreCase.GetHashCode(taskSymbols[i].Name);
                    }
                }
