...
namespace DaggerfallWorkshop.Game
{
    ...
    public class ExteriorAutomap : MonoBehaviour
    {
        ...
        // Multiplayer exterior markers use the same 99900 arrow mesh as the local player.
        // They are still visual-only (no colliders), but now also get a local-style stamp,
        // circle, and name label so they remain easy to spot on a zoomed-out town automap.
        ...
        void Update()
        {
            // The local player cannot normally move while the exterior map window is open,
            // but remote players can. Presence of the automap camera is our window-open signal.
            if (gameObjectCameraExteriorAutomap != null)
                UpdateRemotePlayerMarkers();
        }
        ...
        private Vector3 GetExteriorAutomapMarkerPosition(Vector3 worldPosition)
        {
            Vector3 markerPos;
            float scale = MapsFile.WorldMapTerrainDim * MeshReader.GlobalScale;
            markerPos.x = (worldPosition.x % scale) / scale;
            markerPos.z = (worldPosition.z % scale) / scale;
            markerPos.y = 0.0f;
            ...
            markerPos.x *= refWidth;
            markerPos.y = 0.1f;
            markerPos.z *= refHeight;
            markerPos.x += -(refWidth * 0.5f) + xOffset;
            markerPos.z += -(refHeight * 0.5f) + yOffset;
            return markerPos;
        }

        private void UpdatePlayerMarker()
        {
            // Use one coordinate conversion for both the SP/local marker and remote players.
            gameobjectPlayerMarkerArrow.transform.position = GetExteriorAutomapMarkerPosition(GameManager.Instance.PlayerGPS.transform.position);
        ...
        private RemotePlayerMarkerVisual CreateRemotePlayerMarkerVisual(uint key)
        {
            ...
            marker.arrow = GameObjectHelper.CreateDaggerfallMeshGameObject(99900, gameobjectExteriorAutomap.transform, false, null, true);
            marker.arrow.name = "RemotePlayerMarkerArrow_" + key;
            marker.arrow.layer = layerAutomap;
            Material oldMat = marker.arrow.GetComponent<MeshRenderer>().material;
            Material newMat = new Material(oldMat);
            newMat.shader = Shader.Find("Unlit/Texture");
            marker.arrow.GetComponent<MeshRenderer>().material = newMat;
            marker.arrow.transform.localScale = new Vector3(2.5f, 2.5f, 2.5f) * layoutMultiplier;

            marker.stamp = GameObjectHelper.CreateDaggerfallMeshGameObject(99900, gameobjectExteriorAutomap.transform, false, null, true);
            marker.stamp.name = "RemotePlayerMarkerArrowStamp_" + key;
            marker.stamp.layer = layerAutomap;
            oldMat = marker.stamp.GetComponent<MeshRenderer>().material;
            newMat = new Material(oldMat);
            newMat.shader = Shader.Find("Unlit/Color");
            newMat.color = new Color(0.353f, 0.086f, 0.086f);
            marker.stamp.GetComponent<MeshRenderer>().material = newMat;
            marker.stamp.transform.localScale = new Vector3(4.0f, 4.0f, 4.0f) * layoutMultiplier;

            marker.circle = GameObject.CreatePrimitive(PrimitiveType.Cylinder);
            CapsuleCollider capsuleCollider = marker.circle.GetComponent<CapsuleCollider>();
            if (capsuleCollider)
                UnityEngine.Object.Destroy(capsuleCollider);
            marker.circle.name = "RemotePlayerMarkerCircle_" + key;
            marker.circle.layer = layerAutomap;
            marker.circle.transform.SetParent(gameobjectExteriorAutomap.transform);
            oldMat = marker.circle.GetComponent<MeshRenderer>().material;
            newMat = new Material(oldMat);
            newMat.shader = Shader.Find("Unlit/Color");
            newMat.color = new Color(0.75f, 0.71f, 0.71f);
            marker.circle.GetComponent<MeshRenderer>().material = newMat;
            marker.circle.transform.localScale = new Vector3(12.0f, 1.0f, 12.0f) * layoutMultiplier;

            marker.label = new GameObject("RemotePlayerMarkerLabel_" + key);
            marker.label.transform.SetParent(gameobjectExteriorAutomap.transform);
            ...
            marker.labelText.fontSize = 64;
            marker.labelText.characterSize = 0.75f;
        ...
        private void UpdateRemotePlayerMarkerVisual(RemotePlayerMarkerVisual marker, global::PlayerMultiplayer player)
        {
            if (marker == null || player == null)
                return;

            Vector3 arrowPos = GetExteriorAutomapMarkerPosition(player.transform.position);
            Quaternion arrowRot = player.transform.rotation;

            if (marker.arrow != null)
            {
                marker.arrow.transform.position = arrowPos;
                marker.arrow.transform.rotation = arrowRot;
            }

            if (marker.stamp != null)
            {
                Vector3 stampPos = arrowPos;
                stampPos.y = -10.0f;
                Vector3 biasVec = -Vector3.Normalize(player.transform.forward);
                if (float.IsNaN(biasVec.x) || float.IsNaN(biasVec.y) || float.IsNaN(biasVec.z))
                    biasVec = Vector3.back;
                marker.stamp.transform.position = stampPos + biasVec * 0.8f;
                marker.stamp.transform.rotation = arrowRot;
            }

            if (marker.circle != null)
            {
                Vector3 circlePos = arrowPos;
                circlePos.y = -20.0f;
                marker.circle.transform.position = circlePos;
                marker.circle.transform.rotation = arrowRot;
            }

            if (marker.label != null && marker.labelText != null)
            {
                marker.labelText.text = GetRemotePlayerMarkerName(player);
                Vector3 labelPos = arrowPos;
                labelPos.y = 2.0f;
                marker.label.transform.position = labelPos;
                if (cameraExteriorAutomap != null)
                    marker.label.transform.rotation = cameraExteriorAutomap.transform.rotation;
            }
        }

        /// <summary>
        /// Create/update render-only arrows for remote players occupying the same named
        /// location as the local player. The marker position uses the same exterior layout
        /// conversion as PlayerAdvanced. Markers are visual-only and have no colliders.
        /// </summary>
        private void UpdateRemotePlayerMarkers()
        {
            if (gameobjectExteriorAutomap == null || !location.Loaded || gameobjectPlayerMarkerArrow == null)
                return;
            ...
            for (int i = 0; i < players.Length; i++)
            {
                ...
                UpdateRemotePlayerMarkerVisual(marker, player);
            }
            ...
            foreach (KeyValuePair<uint, RemotePlayerMarkerVisual> pair in remotePlayerMarkers)
            {
                ...
                if (pair.Value != null)
                {
                    ...
                    if (pair.Value.stamp != null)
                        UnityEngine.Object.Destroy(pair.Value.stamp);
                    if (pair.Value.circle != null)
                        UnityEngine.Object.Destroy(pair.Value.circle);
        ...
        private void DestroyRemotePlayerMarkers()
        {
            foreach (KeyValuePair<uint, RemotePlayerMarkerVisual> pair in remotePlayerMarkers)
            {
                if (pair.Value == null)
