...
namespace DaggerfallWorkshop.Game.UserInterfaceWindows
{
    public class DaggerfallQuestJournalWindow : DaggerfallPopupWindow
    {
        ...
        const string shareQuestText = "[Share Quest]";
        protected Dictionary<int, int> questShareLineMap = new Dictionary<int, int>();
        ...
        {
            if (entryLineMap == null || entryLineMap.Count == 0)
                return;
            ...
            if (DisplayMode == JournalDisplay.ActiveQuests && TryHandleQuestShareClick(line))
                return;
            ...
            {
                if (selectedEntry >= 0 && selectedEntry < questMessages.Count)
        ...
        protected virtual bool TryHandleQuestShareClick(int line)
        {
            if (questShareLineMap == null || questShareLineMap.Count == 0)
                return false;

            int questMessageIndex;
            if (!questShareLineMap.TryGetValue(line, out questMessageIndex))
                return false;

            if (questMessages == null || questMessageIndex < 0 || questMessageIndex >= questMessages.Count)
                return false;

            selectedEntry = questMessageIndex;
            ShareQuestFromJournal(questMessages[questMessageIndex]);
            return true;
        }

        protected virtual void ShareQuestFromJournal(Message questMessage)
        {
            DaggerfallUI.Instance.PlayOneShot(SoundClips.ButtonClick);

            if (questMessage == null || questMessage.ParentQuest == null)
            {
                ShowQuestShareStatus("No quest found for this journal entry.");
                return;
            }
            ...
            try
            {
                shared = global::QuestNetSync.TryShareQuestFromJournal(questMessage.ParentQuest.UID, out statusMessage);
            }
            catch (System.Exception ex)
            {
                statusMessage = "Quest share failed. See the Unity log for details.";
                Debug.LogWarning("[QuestJournal] Quest share failed: " + ex);
            }

            if (string.IsNullOrEmpty(statusMessage))
                statusMessage = shared ? "Quest share request sent." : "Quest was not shared.";

            ShowQuestShareStatus(statusMessage);
        }

        protected virtual void ShowQuestShareStatus(string statusMessage)
        {
            if (string.IsNullOrEmpty(statusMessage))
                return;

            TextFile.Token[] tokens = new TextFile.Token[] {
                TextFile.CreateTextToken("Quest Share"), TextFile.CreateFormatToken(TextFile.Formatting.JustifyCenter), TextFile.NewLineToken,
                TextFile.CreateTextToken(statusMessage), TextFile.CreateFormatToken(TextFile.Formatting.EndOfRecord)
            };
            ...
            dialogBox.AddButton(DaggerfallMessageBox.MessageBoxButtons.OK);
            dialogBox.OnButtonClick += QuestShareStatus_OnButtonClick;
            DaggerfallUI.UIManager.PushWindow(dialogBox);
        }

        protected virtual void QuestShareStatus_OnButtonClick(DaggerfallMessageBox sender, DaggerfallMessageBox.MessageBoxButtons messageBoxButton)
        {
            sender.CloseWindow();
        }
        ...
        {
            messageCount = questMessages != null ? questMessages.Count : 0;
            ...
            questShareLineMap.Clear();
            bool showShareLinks = global::QuestNetSync.IsQuestShareAvailableForJournal();
            ...
            for (int i = currentMessageIndex; i < questMessages.Count; i++)
            {
                ...
                bool showShareForThisQuest = showShareLinks &&
                    questMessages[i] != null &&
                    questMessages[i].ParentQuest != null &&
                    !global::QuestNetSync.IsQuestSharingBlacklisted(questMessages[i].ParentQuest);

                if (showShareForThisQuest)
                {
                    TextFile.Token shareToken = new TextFile.Token() { text = shareQuestText, formatting = TextFile.Formatting.TextHighlight };
                    questShareLineMap[totalLineCount] = i;
                    textTokens.Add(shareToken);
                    totalLineCount++;
                    entryLineMap.Add(i);
                    ...
                    // Extra blank line so the share link does not replace the normal
                    // visual spacing between separate active quest entries.
                    if (totalLineCount >= maxLinesQuests)
                        break;
                ...
                {
                    // Keep the vanilla spacer line between active quests when MP sharing
                    // is not available, and do not expose a dead share link in SP.
                    textTokens.Add(TextFile.NewLineToken);
                    totalLineCount++;
                    entryLineMap.Add(i);
                }
