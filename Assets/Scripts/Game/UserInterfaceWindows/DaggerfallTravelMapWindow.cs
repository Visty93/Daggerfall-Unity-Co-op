...
namespace DaggerfallWorkshop.Game.UserInterfaceWindows
{
    ...
    public class DaggerfallTravelMapWindow : DaggerfallPopupWindow
    {
        ...
        // Multiplayer party world-map positions use their own transparent overlay so they
        // never overwrite DFU's normal Find / "I'm At" identify texture. The overlay does
        // not have its own animation loop: it follows the existing temporary identify flash.
        protected Panel partyPositionOverlayPanel;

        protected TextLabel regionLabel;

        protected Texture2D overworldTexture;
        protected Texture2D identifyTexture;
        protected Texture2D partyPositionTexture;
        ...
        protected Color32[] partyPositionPixelBuffer;
        ...
        protected readonly Color32 partyPositionColor = new Color32(0, 220, 255, 230);

        // Remote regional-map markers are intentionally small. Full-width crosshairs for every
        // player quickly cover location dots and names when several people share a region.
        protected const int partyPositionMarkerHalfSize = 4;
        protected const int partyPositionClusterDistance = 3;
        protected bool partyPositionIdentifyActive = false;

        protected sealed class PartyPositionNameMarker
        {
        ...
        protected readonly List<PartyPositionNameMarker> partyPositionNameMarkers = new List<PartyPositionNameMarker>();
        ...
        protected override void Setup()
        {
            ...
            // Independent multiplayer party-position overlay. It is only enabled during the
            // same short identify flash used by the normal map, never as a permanent HUD layer.
            partyPositionOverlayPanel = DaggerfallUI.AddPanel(regionTextureOverlayPanelRect, NativePanel);
            partyPositionOverlayPanel.Enabled = false;
            ...
            partyPositionPixelBuffer = new Color32[(int)regionTextureOverlayPanelRect.width * (int)regionTextureOverlayPanelRect.height];
            partyPositionTexture = new Texture2D((int)regionTextureOverlayPanelRect.width, (int)regionTextureOverlayPanelRect.height, TextureFormat.ARGB32, false);
            partyPositionTexture.filterMode = FilterMode.Point;
            partyPositionOverlayPanel.BackgroundTexture = partyPositionTexture;
        ...
        public override void OnPop()
        {
            ...
            distance = null;
            partyPositionIdentifyActive = false;
            SetPartyPositionVisibility(false);
        }
        ...
        public override void Update()
        {
            ...
            // Show/hide identify panels when identify is running. Party positions deliberately
            // follow this exact temporary flash state instead of maintaining their own animation.
            identifyOverlayPanel.Enabled = identifying && identifyState;
            SetPartyPositionVisibility(partyPositionIdentifyActive && identifying && identifyState);
            AnimateIdentify();
        ...
        protected virtual void ZoomMapTextures()
        {
            ...
            if (!RegionSelected || !zoom)
            {
                ...
                partyPositionOverlayPanel.BackgroundTextureLayout = BackgroundLayout.StretchToFill;
                UpdatePartyPositionLabelPositions();
                UpdateBorder();
                return;
            }
            ...
            partyPositionOverlayPanel.BackgroundTextureLayout = BackgroundLayout.Cropped;
            partyPositionOverlayPanel.BackgroundCroppedRect = regionLocationDotsOverlayPanel.BackgroundCroppedRect;
            UpdatePartyPositionLabelPositions();
        ...
        protected virtual void UpdateIdentifyTextureForPlayerRegion()
        {
            ...
            if (partyPositionIdentifyActive)
                RefreshPartyPositionOverlay();
        ...
        {
            // Identify local position and connected party positions together. Party markers
            // remain temporary and disappear with the normal "I'm At" flash.
            findingLocation = false;
            StartPartyIdentify();
            UpdateCrosshair();
        }
        ...
        void StartIdentify()
        {
            ...
            // Normal DFU identify operations (Find, journal destination, etc.) do not implicitly
            // show party positions. Only map-open/navigation and "I'm At" opt into them.
            partyPositionIdentifyActive = false;
            SetPartyPositionVisibility(false);
        ...
        void StartPartyIdentify()
        {
            StartIdentify();
            partyPositionIdentifyActive = true;
        }
        ...
        void SetPartyPositionVisibility(bool visible)
        {
            if (partyPositionOverlayPanel != null)
                partyPositionOverlayPanel.Enabled = visible;

            bool showLabels = visible && RegionSelected;
            for (int i = 0; i < partyPositionNameMarkers.Count; i++)
            {
                PartyPositionNameMarker marker = partyPositionNameMarkers[i];
                if (marker == null || marker.label == null)
                    continue;

                marker.label.Enabled = showLabels && marker.used && IsPartyPositionLabelInsideVisibleMap(marker);
            }
        }

        void RefreshPartyPositionOverlay()
        {
            if (partyPositionPixelBuffer == null || partyPositionTexture == null || partyPositionOverlayPanel == null)
                return;

            Array.Clear(partyPositionPixelBuffer, 0, partyPositionPixelBuffer.Length);
            for (int i = 0; i < partyPositionNameMarkers.Count; i++)
            {
                if (partyPositionNameMarkers[i] != null)
                {
                    partyPositionNameMarkers[i].used = false;
                    if (partyPositionNameMarkers[i].label != null)
                        partyPositionNameMarkers[i].label.Enabled = false;
                }
            }

            if (RegionSelected)
                DrawPartyPositionsOnRegionMap();
            else
                DrawPartyRegionsOnWorldMap();

            partyPositionTexture.SetPixels32(partyPositionPixelBuffer);
            partyPositionTexture.Apply();
            partyPositionOverlayPanel.BackgroundTexture = partyPositionTexture;
            UpdatePartyPositionLabelPositions();
        }

        void DrawPartyRegionsOnWorldMap()
        {
            int localRegion = GetPlayerRegion();
            HashSet<int> remoteRegions = new HashSet<int>();
            ...
            for (int i = 0; i < players.Length; i++)
            {
                ...
                if (player == null || !player.isActiveAndEnabled || player.isLocalPlayer)
                    continue;

                DFPosition mapPixel;
                int remoteRegion;
                string playerName;
                if (!TryGetPartyTravelMapPosition(player, out mapPixel, out remoteRegion, out playerName))
                    continue;

                // The normal local red flash already communicates this region. Adding a cyan
                // fill on top would only make the same region harder to read.
                if (remoteRegion != localRegion)
            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    int srcOffset = y * width + x;
                    int sampleRegion = regionPickerBitmap.Data[srcOffset] - 128;
                    if (!remoteRegions.Contains(sampleRegion))
                        continue;

                    int dstOffset = ((height - y - pickerOverlayPanelHeightDifference) * width) + x;
                    if (dstOffset >= 0 && dstOffset < partyPositionPixelBuffer.Length)
                        partyPositionPixelBuffer[dstOffset] = partyPositionColor;
                }
            }
        }

        void DrawPartyPositionsOnRegionMap()
        {
            if (!RegionSelected || selectedRegionMapNames == null || mapIndex < 0 || mapIndex >= selectedRegionMapNames.Length)
                return;
            ...
            float mapScale = GetRegionMapScale(selectedRegion);
            int yAdjust = selectedRegion == betonyIndex ? -477 : 0;
            int width = (int)regionTextureOverlayPanelRect.width;
            int height = (int)regionTextureOverlayPanelRect.height;
            int usedLabelCount = 0;
            List<Vector2> markerCenters = new List<Vector2>();
            ...
            for (int i = 0; i < players.Length; i++)
            {
                ...
                int remoteRegion;
                string playerName;
                if (!TryGetPartyTravelMapPosition(player, out mapPixel, out remoteRegion, out playerName) || remoteRegion != selectedRegion)
                    continue;

                int displayX = (int)((mapPixel.X - origin.x) * mapScale);
                int scaledY = (int)((mapPixel.Y - origin.y) * mapScale) + regionPanelOffset + yAdjust;
                int displayY = scaledY - regionPanelOffset;

                // This also filters players who are in the same political region but belong on
                // another FMAP page of a multi-page region.
                if (displayX < 0 || displayX >= width || displayY < 0 || displayY >= height)
                    continue;

                DrawPartyPositionMarker(displayX, displayY);

                if (!string.IsNullOrEmpty(playerName))
                {
                    int stackIndex = 0;
                    for (int c = 0; c < markerCenters.Count; c++)
                    {
                        if (Mathf.Abs(markerCenters[c].x - displayX) <= partyPositionClusterDistance &&
                            Mathf.Abs(markerCenters[c].y - displayY) <= partyPositionClusterDistance)
                            stackIndex++;
                    }
                    markerCenters.Add(new Vector2(displayX, displayY));
                    ConfigurePartyPositionNameMarker(usedLabelCount++, playerName, displayX, displayY, stackIndex);
                }
            }

            for (int i = usedLabelCount; i < partyPositionNameMarkers.Count; i++)
            {
                PartyPositionNameMarker marker = partyPositionNameMarkers[i];
                if (marker != null)
                {
        ...
        bool TryGetPartyTravelMapPosition(global::PlayerMultiplayer player, out DFPosition mapPixel, out int region, out string playerName)
        {
            ...
            global::PositionMultiplayer position = player.GetComponent<global::PositionMultiplayer>();
            if (position == null || position.x <= 0 || position.z <= 0)
                return false;

            mapPixel = MapsFile.WorldCoordToMapPixel(position.x, position.z);
            if (mapPixel.X < TerrainHelper.minMapPixelX || mapPixel.X > TerrainHelper.maxMapPixelX ||
                mapPixel.Y < TerrainHelper.minMapPixelY || mapPixel.Y > TerrainHelper.maxMapPixelY)
                return false;

            region = DaggerfallUnity.Instance.ContentReader.MapFileReader.GetPoliticIndex(mapPixel.X, mapPixel.Y) - 128;
            if (region < 0 || region >= DaggerfallUnity.Instance.ContentReader.MapFileReader.RegionCount)
                return false;

            global::PlayerAssets assets = player.GetComponent<global::PlayerAssets>();
            if (assets != null && !string.IsNullOrEmpty(assets.playerName) &&
                !string.Equals(assets.playerName, "Nameless", StringComparison.OrdinalIgnoreCase))
                playerName = assets.playerName;

            return true;
        }

        void DrawPartyPositionMarker(int displayX, int displayY)
        {
            for (int d = -partyPositionMarkerHalfSize; d <= partyPositionMarkerHalfSize; d++)
            {
                SetPartyPositionPixel(displayX + d, displayY);
                SetPartyPositionPixel(displayX, displayY + d);
            }

            // Slightly strengthen the centre so the marker remains easy to spot when zoomed out.
            SetPartyPositionPixel(displayX - 1, displayY);
            SetPartyPositionPixel(displayX + 1, displayY);
            SetPartyPositionPixel(displayX, displayY - 1);
            SetPartyPositionPixel(displayX, displayY + 1);
        }

        void SetPartyPositionPixel(int displayX, int displayY)
        {
            int width = (int)regionTextureOverlayPanelRect.width;
            int height = (int)regionTextureOverlayPanelRect.height;
            if (displayX < 0 || displayX >= width || displayY < 0 || displayY >= height)
            ...
            int textureY = height - displayY - 1;
            int offset = textureY * width + displayX;
            if (offset >= 0 && offset < partyPositionPixelBuffer.Length)
                partyPositionPixelBuffer[offset] = partyPositionColor;
        }

        void ConfigurePartyPositionNameMarker(int index, string playerName, int displayX, int displayY, int stackIndex)
        {
            while (partyPositionNameMarkers.Count <= index)
            {
                TextLabel label = DaggerfallUI.AddTextLabel(DaggerfallUI.DefaultFont, Vector2.zero, string.Empty, NativePanel);
                label.Enabled = false;
                label.ShadowColor = Color.black;
                partyPositionNameMarkers.Add(new PartyPositionNameMarker() { label = label });
            }

            PartyPositionNameMarker marker = partyPositionNameMarkers[index];
            marker.label.Text = playerName;
        ...
        void UpdatePartyPositionLabelPositions()
        {
            if (partyPositionNameMarkers.Count == 0)
                return;
            ...
            if (RegionSelected && zoom)
            {
                crop = partyPositionOverlayPanel.BackgroundCroppedRect;
                if (crop.width > 0)
                    xScale = width / crop.width;
                if (crop.height > 0)
                    yScale = height / crop.height;
            }

            for (int i = 0; i < partyPositionNameMarkers.Count; i++)
            {
                PartyPositionNameMarker marker = partyPositionNameMarkers[i];
                if (marker == null || marker.label == null || !marker.used)
                    continue;

                int textureY = height - marker.displayY - 1;
                float screenX = (marker.displayX - crop.x) * xScale;
                float screenY = regionPanelOffset + (crop.height - 1f - (textureY - crop.y)) * yScale;

                // Put the name just above/right of the marker. Players occupying effectively the
                // same map pixel stack vertically rather than painting names over one another.
                screenX += 5f;
                screenY -= 8f + marker.stackIndex * 8f;

                marker.label.Position = new Vector2(
                    Mathf.Clamp(screenX, 0f, regionTextureOverlayPanelRect.width - 8f),
                    Mathf.Clamp(screenY, regionPanelOffset, regionPanelOffset + regionTextureOverlayPanelRect.height - 8f));
            }

            SetPartyPositionVisibility(partyPositionIdentifyActive && identifying && identifyState);
        }

        bool IsPartyPositionLabelInsideVisibleMap(PartyPositionNameMarker marker)
        {
            if (marker == null || !marker.used || !RegionSelected)
                return false;

            if (!zoom)
                return marker.displayX >= 0 && marker.displayX < regionTextureOverlayPanelRect.width &&
                       marker.displayY >= 0 && marker.displayY < regionTextureOverlayPanelRect.height;

            int height = (int)regionTextureOverlayPanelRect.height;
            int textureY = height - marker.displayY - 1;
            Rect crop = partyPositionOverlayPanel.BackgroundCroppedRect;
            return marker.displayX >= crop.x && marker.displayX < crop.xMax &&
                   textureY >= crop.y && textureY < crop.yMax;
        }
