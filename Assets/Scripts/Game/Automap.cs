...
namespace DaggerfallWorkshop.Game
{
    ...
    public class Automap : MonoBehaviour
    {
        ...
        private class RemotePlayerMarkerVisual
        {
        ...
        // Multiplayer automap markers are render-only copies of the normal player arrow.
        // They deliberately have no colliders and use the live remote NetworkTransform for
        // exact position/rotation. PositionMultiplayer only decides same-map eligibility.
        readonly Dictionary<uint, RemotePlayerMarkerVisual> remotePlayerMarkers = new Dictionary<uint, RemotePlayerMarkerVisual>();
        ...
        public void UpdateAutomapStateOnWindowPush()
        {
            ...
            // Reconcile multiplayer arrows immediately when the map opens. Remote players
            // can continue moving while this window is open, so Update() keeps them live.
...
if (isOpenAutomap) // only do this stuff if automap is indeed open
{
    ...
    {
        Debug.LogWarning("[Automap] gameobjectBeaconRotationPivotAxis is null in Update().");
    }
}
}
...
DaggerfallDungeon dungeon = GameObject.FindObjectsOfType<DaggerfallDungeon>()
    .FirstOrDefault(x => x.isActiveAndEnabled);
        ...
        private static uint GetRemotePlayerMarkerKey(global::PlayerMultiplayer player)
        {
            if (player == null)
                return 0;

            return player.netId != 0
                ? player.netId
                : unchecked((uint)player.GetInstanceID());
        }

        private static string GetRemotePlayerMarkerName(global::PlayerMultiplayer player)
        {
            if (player == null)
                return string.Empty;
            ...
            return assets.playerName;
        }

        private RemotePlayerMarkerVisual CreateRemotePlayerMarkerVisual(uint key)
        {
            RemotePlayerMarkerVisual marker = new RemotePlayerMarkerVisual();

            marker.arrow = GameObjectHelper.CreateDaggerfallMeshGameObject(99900, gameobjectBeacons.transform, false, null, true);
            marker.arrow.name = "RemotePlayerMarkerArrow_" + key;
            marker.arrow.layer = layerAutomap;
            // Intentionally no MeshCollider: remote party markers are visual-only.

            marker.label = new GameObject("RemotePlayerMarkerLabel_" + key);
            marker.label.transform.SetParent(gameobjectBeacons.transform);
            marker.label.layer = layerAutomap;
            marker.labelText = marker.label.AddComponent<TextMesh>();
            marker.labelText.anchor = TextAnchor.LowerCenter;
            marker.labelText.alignment = TextAlignment.Center;
            marker.labelText.fontSize = 32;
            marker.labelText.characterSize = 0.12f;
            marker.labelText.text = string.Empty;
            Font labelFont = Resources.GetBuiltinResource<Font>("Arial.ttf");
            if (labelFont != null)
                marker.labelText.font = labelFont;
            MeshRenderer labelRenderer = marker.label.GetComponent<MeshRenderer>();
            if (labelRenderer != null)
            {
                Material labelMaterial = labelFont != null ? new Material(labelFont.material) : new Material(Shader.Find("GUI/Text Shader"));
                labelMaterial.color = new Color(1.0f, 0.95f, 0.35f, 1.0f);
        ...
        private void UpdateRemotePlayerMarkerLabel(RemotePlayerMarkerVisual marker, string playerName, Vector3 arrowPosition)
        {
            if (marker == null || marker.label == null || marker.labelText == null)
                return;

            marker.labelText.text = string.IsNullOrEmpty(playerName) ? "Player" : playerName;
            marker.label.transform.position = arrowPosition + Vector3.up * 1.35f;
            if (cameraAutomap != null)
                marker.label.transform.rotation = cameraAutomap.transform.rotation;
        }

        /// <summary>
        /// Create/update render-only arrows for remote players occupying this exact interior
        /// or network dungeon. No collider is added, so these markers never participate in
        /// automap raycasts or hover hit testing.
        /// </summary>
        private void UpdateRemotePlayerMarkers()
        {
            if (!isOpenAutomap || gameobjectBeacons == null)
                return;

            global::PlayerMultiplayer[] players = UnityEngine.Object.FindObjectsOfType<global::PlayerMultiplayer>();
            HashSet<uint> liveKeys = new HashSet<uint>();
            ...
            for (int i = 0; i < players.Length; i++)
            {
                ...
                if (position == null || !position.IsInSameAutomapSpaceAsLocalPlayer())
                    continue;

                uint key = GetRemotePlayerMarkerKey(player);
                liveKeys.Add(key);

                RemotePlayerMarkerVisual marker;
                if (!remotePlayerMarkers.TryGetValue(key, out marker) || marker == null || marker.arrow == null)
                {
                    marker = CreateRemotePlayerMarkerVisual(key);
                    remotePlayerMarkers[key] = marker;
                }

                marker.arrow.transform.position = player.transform.position;
                marker.arrow.transform.rotation = player.transform.rotation;
                UpdateRemotePlayerMarkerLabel(marker, GetRemotePlayerMarkerName(player), marker.arrow.transform.position);
            }

            if (remotePlayerMarkers.Count == 0)
                return;

            List<uint> staleKeys = null;
            foreach (KeyValuePair<uint, RemotePlayerMarkerVisual> pair in remotePlayerMarkers)
            {
                ...
                if (pair.Value != null)
                {
                    if (pair.Value.arrow != null)
                        UnityEngine.Object.Destroy(pair.Value.arrow);
                    if (pair.Value.label != null)
                        UnityEngine.Object.Destroy(pair.Value.label);
                }

                if (staleKeys == null)
                    staleKeys = new List<uint>();
            ...
            if (staleKeys != null)
            {
                for (int i = 0; i < staleKeys.Count; i++)
                    remotePlayerMarkers.Remove(staleKeys[i]);
            }
        }
...

        {
            ...
            // Remote arrows are children of Beacons and were destroyed with their parent.
            // Clear cached Unity references immediately so a recreated map cannot reuse them.
...
if (dungeon != null && dungeon.StartMarker != null)
{
    ...
    int entranceX = originX * microMapBlockSizeInPixels + (int)(Mathf.Floor(entrancePosX * 2)) * (microMapBlockSizeInPixels / 2);
    int entranceY = originY * microMapBlockSizeInPixels + (int)(Mathf.Floor(entrancePosY * 2)) * (microMapBlockSizeInPixels / 2);
    for (int y = 0; y < microMapBlockSizeInPixels / 2; y++)
    {
        for (int x = 0; x < microMapBlockSizeInPixels / 2; x++)
        {
            textureMicroMap.SetPixel(entranceX + x, entranceY + y, Color.green);
        }
    }
}
else
{
    Debug.LogWarning("[Automap] Dungeon or StartMarker not found for micro map entrance marking.");
}
...
int playerX = originX * microMapBlockSizeInPixels + (int)(Mathf.Floor(playerPosX * 2)) * (microMapBlockSizeInPixels / 2);
int playerY = originY * microMapBlockSizeInPixels + (int)(Mathf.Floor(playerPosY * 2)) * (microMapBlockSizeInPixels / 2);
for (int y = 0; y < microMapBlockSizeInPixels / 2; y++)
{
    for (int x = 0; x < microMapBlockSizeInPixels / 2; x++)
    {
        textureMicroMap.SetPixel(playerX + x, playerY + y, Color.red);
    }
}
        ...
        private void CreateDungeonGeometryForAutomap()
        {
            ...
            // Find any active DaggerfallDungeon in the scene
DaggerfallDungeon dungeon = GameObject.FindObjectsOfType<DaggerfallDungeon>()
    .FirstOrDefault(x => x.isActiveAndEnabled && x.Summary.LocationName == location.Name);

if (dungeon != null)
{
    DFLocation.DungeonBlock[] blocks = dungeon.Summary.LocationData.Dungeon.Blocks;
    ...
    {
        DFBlock blockData;
        int[] textureTable = dungeon.DungeonTextureTable;
    ...
    // Copy position and rotation from dungeon prefab
    gameobjectGeometry.transform.position = dungeon.transform.position;
    gameobjectGeometry.transform.rotation = dungeon.transform.rotation;

    SetupBeacons(); // ✅ This now works
}
else
{
    Debug.LogWarning("[Automap] No active dungeon found for automap setup.");
}
        ...
        private void SaveStateAutomapDungeon(bool forceSaveOfDungeonDiscoveryState)
{
    ...
    if ((!GameManager.Instance.IsPlayerInside) && (!forceSaveOfDungeonDiscoveryState) || gameobjectGeometry == null)
    {
        return;
    }

    // ✅ Safety check to prevent UnityException from accessing missing child
    if (gameobjectGeometry.transform.childCount == 0)
    {
        Debug.LogWarning("[Automap] gameobjectGeometry is null or has no children. Skipping SaveStateAutomapDungeon.");
        return;
    }
    ...
    automapDungeonState.entranceDiscovered = ((gameobjectBeaconEntrancePosition) && (gameobjectBeaconEntrancePosition.activeSelf));
        ...
        foreach (Transform currentTransformModel in currentBlock.transform)
        {
            ...
            foreach (Transform currentTransformMesh in currentTransformModel.transform)
            {
                ...
                for (int i = 0; i < meshRenderers.Length; i++)
                {
                    ...
                    {
                        model.discovered = true;
                        model.visitedInThisRun = !meshRenderers[i].materials[0].IsKeywordEnabled("RENDER_IN_GRAYSCALE");
                        break;
                    }
                    else
                    {
    ...
    if (sortedList.Count > updatedNumberOfDungeonMemorized)
    {
        ...
        foreach (KeyValuePair<string, AutomapDungeonState> entry in sortedList)
        {
            if (entry.Value.timeInSecondsLastVisited < timeInSecondsLimit)
            {
                dictAutomapDungeonsDiscoveryState.Remove(entry.Key);
            }
        }
    }
}
...
void InitWhenInInteriorOrDungeon(StaticDoor? door = null, bool initFromLoadingSave = false)
{
    if ((GameManager.Instance.IsPlayerInsideBuilding) && (door.HasValue))
    {
        ...
        {
            Debug.LogWarning("[Automap] gameobjectGeometry not initialized after CreateIndoorGeometryForAutomap.");
        }
    ...
    else if ((GameManager.Instance.IsPlayerInsideDungeon) || (GameManager.Instance.IsPlayerInsideCastle))
    {
        ...
        if (gameobjectGeometry)
        {
            RestoreStateAutomapDungeon(!initFromLoadingSave);
        }
        else
        {
            Debug.LogWarning("[Automap] gameobjectGeometry not initialized after CreateDungeonGeometryForAutomap.");
        }
    ...
    {
        Debug.LogError("Error in InitWhenInInteriorOrDungeon: invalid control flow.");
    }
}
