...
namespace DaggerfallWorkshop.Game
{
    ...
    public class EnemySounds : MonoBehaviour
    {
        ...
        public bool PlayAttackSound()
        {
            ...
            // Play attack sound only about half the time. Return whether it actually played
            // so networked attack animation RPCs can reproduce the same sound decision
            // on observing clients instead of rolling a different random result.
            if (Random.value > 0.5f)
            {
                PlayAttackSoundForced();
                return true;
            }
        ...
        public void PlayAttackSoundForced()
        {
            if (!IsReady())
                return;
        ...
        public void PlayGenericHitSound(bool weaponHit)
        {
            if (!IsReady())
                return;

            int sound = weaponHit
                ? (int)SoundClips.Hit1 + Random.Range(0, 5)
                : (int)SoundClips.Hit1 + Random.Range(2, 4);
