...
namespace DaggerfallWorkshop.Game
{
    ...
    public class PlayerFootsteps : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            ...
            // Multiplayer dungeons can be vertically offset, so do not compare against raw
            // blockWaterLevel here. PlayerEnterExit owns the offset-aware surface calculation.
            if (playerEnterExit != null && playerEnterExit.IsPlayerInsideDungeon && playerEnterExit.blockWaterLevel != 10000)
            {
                // In water, deep depth. This flag is already calculated by PlayerEnterExit
                // with the network dungeon Y offset applied.
                ...
                // In water, shallow depth. Use the same offset-aware dungeon water surface
                // as PlayerEnterExit, otherwise every negative-Y MP dungeon sounds wet.
                else if ((currentFootstepSound1 != FootstepSoundShallow) &&
                         playerEnterExit.ShouldUseDungeonShallowFootstepSound(playerMotor.transform.position.y - 0.57f))
                {
            ...
            // Only query the offset-aware dungeon water surface while a dungeon-water
            // footstep sound is actually active. The previous version called into
            // PlayerEnterExit every FixedUpdate, even while using ordinary stone footsteps.
            // In a large network dungeon that could repeatedly scan the whole dungeon
            // renderer hierarchy when no visual water renderer was found.
            bool usingDungeonWaterFootsteps =
                currentFootstepSound1 == FootstepSoundSubmerged ||
                currentFootstepSound1 == FootstepSoundShallow;

            // Not in water, reset footsteps to normal.
            // Keep this short-circuited just like the original SP implementation.
            if (!playerOnExteriorWater && usingDungeonWaterFootsteps)
            {
                bool shouldResetDungeonWaterFootsteps =
                    playerEnterExit == null ||
                    playerEnterExit.ShouldResetDungeonWaterFootstepSound(
                        playerMotor.transform.position.y - 0.95f);

                if (shouldResetDungeonWaterFootsteps)
                {
                    currentFootstepSound1 = FootstepSoundDungeon1;
                    currentFootstepSound2 = FootstepSoundDungeon2;
