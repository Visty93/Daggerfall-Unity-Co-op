...
namespace DaggerfallWorkshop.Game
{
    ...
    public class WeatherManager : MonoBehaviour
    {
        ...
        // Dungeon/interior fog sets black RenderSettings fog and only restores cached outdoor fog on exit.
        // When the actual outdoor weather is Fog, the cache can be correct as an enum but wrong visually
        // until the next natural weather refresh. Reapply the current outdoor weather shortly after
        // exterior transition so heavy fog/sky fog color is restored immediately.
        Coroutine delayedExteriorWeatherReapplyCoroutine = null;
        ...
        void OnTransitionToExterior(PlayerEnterExit.TransitionEventArgs args)
        {
            ...
            ScheduleExteriorWeatherVisualReapply();
        }

        void ScheduleExteriorWeatherVisualReapply()
        {
            if (delayedExteriorWeatherReapplyCoroutine != null)
                StopCoroutine(delayedExteriorWeatherReapplyCoroutine);

            delayedExteriorWeatherReapplyCoroutine = StartCoroutine(DelayedExteriorWeatherVisualReapply());
        }

        IEnumerator DelayedExteriorWeatherVisualReapply()
        {
            // Let PlayerEnterExit finish enabling the exterior parent and let the sky camera update once.
            // Reapplying too early can just restore the dungeon black fog again.
            yield return null;
            yield return new WaitForEndOfFrame();

            ReapplyCurrentOutdoorWeatherVisualsAfterExteriorTransition("initial");

            // A second short pass catches cases where the exterior/sky became ready one frame later
            // after dungeon exit, fast travel, or MP network dungeon teardown.
            yield return new WaitForSeconds(0.25f);
            ReapplyCurrentOutdoorWeatherVisualsAfterExteriorTransition("delayed");

            delayedExteriorWeatherReapplyCoroutine = null;
        }

        void ReapplyCurrentOutdoorWeatherVisualsAfterExteriorTransition(string passName)
        {
            ...
            WeatherType currentWeather = PlayerWeather.WeatherType;

            Debug.Log($"[WeatherManager] Reapply exterior weather visuals ({passName}) weather={currentWeather} fogMode={currentOutdoorFogSettings.fogMode} density={currentOutdoorFogSettings.density}");

            // This is intentionally a visual reapply, even if the enum did not change.
            // For Fog this reruns SetRainOvercast() + SetFog(HeavyFogSettings), restoring
            // the close fog and sky-fog color immediately after dungeon black fog.
            SetWeather(currentWeather);
            RefreshSkyFogColorAfterWeatherReapply();
        }

        void RefreshSkyFogColorAfterWeatherReapply()
        {
            // DaggerfallSky normally refreshes RenderSettings.fogColor when sky textures update.
            // After dungeon exit the weather enum may be unchanged, so force its fog-color rule now.
            if (DaggerfallSky != null &&
                DaggerfallSky.skyColors.west != null &&
                DaggerfallSky.skyColors.west.Length > 0)
            {
                DaggerfallSky.SetSkyFogColor(DaggerfallSky.skyColors);
                return;
            }

            // Safe fallback if sky textures are not ready yet. Heavy outdoor fog should not keep
            // the dungeon's black fog color. This mirrors DaggerfallSky.SetSkyFogColor().
            if (currentOutdoorFogSettings.fogMode == FogMode.Exponential &&
                currentOutdoorFogSettings.density > RainyFogSettings.density)
            {
                RenderSettings.fogColor = Color.gray;
            }
            else
