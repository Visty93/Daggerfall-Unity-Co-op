...
namespace DaggerfallWorkshop.Game.MagicAndEffects
{
    ...
    /// </summary>
    public class EntityEffectManager : NetworkBehaviour
    {
        #region Fields
        ...
        Debug.Log($"[EntityEffectManager] Attached to entity: {entityBehaviour.name} ({entityType})");
            }
			else
    {
        Debug.LogError("[EntityEffectManager] ERROR: No DaggerfallEntityBehaviour found!");
    }
            ...
            if (IsPlayerEntity)
			 ...
			 Debug.Log("[EntityEffectManager] PlayerSpellCasting event connected.");
			 }
        ...
        private bool PlayerHasEnoughSpellPointsForReadySpell()
        {
            if (!IsPlayerEntity || readySpell == null || readySpellDoesNotCostSpellPoints)
                return true;

            if (GameManager.Instance != null && GameManager.Instance.PlayerEntity != null && GameManager.Instance.PlayerEntity.GodMode)
                return true;

            return entityBehaviour != null && entityBehaviour.Entity != null && entityBehaviour.Entity.CurrentMagicka >= readySpellCastingCost;
        }

        private void ClearReadySpellAfterFailedCast()
        {
            DaggerfallUI.AddHUDText(TextManager.Instance.GetLocalizedText(youDontHaveTheSpellPointsMessageKey));
            AbortReadySpell();
        }
        ...
        public void CastNoAnimSpell()
        {
            ...
            // A ready spell can become invalid if the player's magicka changes after
            // selecting it. Clear it cleanly instead of leaving the cast/icon armed.
            if (!PlayerHasEnoughSpellPointsForReadySpell())
            {
                ClearReadySpellAfterFailedCast();
                return;
            }

            // Report a visual-only version of this player cast to other clients.
            // This is cosmetic only; real local payload/effect handling continues below.
            ReportLocalPlayerSpellCastVisual(readySpell);
        ...
        public void CastReadySpell()
        {
            ...
            // In multiplayer, a remote PlayerMultiplayer root CharacterController is also
            // a valid friendly spell target even though it does not have a real
            // DaggerfallEntityBehaviour / EntityEffectManager. The actual effect is
            // forwarded to the target owner's local PlayerAdvanced later by DaggerfallMissile.
            if (IsPlayerEntity && readySpell.Settings.TargetType == TargetTypes.ByTouch)
            {
                ...
                global::PlayerMultiplayer playerTarget;
                bool hasPlayerMultiplayerTarget = global::PlayerSpellMultiplayerBridge.TryGetPlayerMultiplayerTargetInTouchRange(aimPosition, aimDirection, out playerTarget);
                if (hasPlayerMultiplayerTarget)
                {
                    // If the touch ray is on a PlayerMultiplayer shell, only allow the
                    // cast to release when the bundle is allowed as a friendly player spell.
                    // This prevents hostile touch spells from spending magicka on the fake
                    // visual enemy child without actually affecting the real player.
                    if (!global::PlayerSpellMultiplayerBridge.CanForwardLocalFriendlyPlayerSpell(readySpell, playerTarget))
                        return;
                }
                else if (DaggerfallMissile.GetEntityTargetInTouchRange(aimPosition, aimDirection) == null)
                {
        ...
        /// <summary>
        /// Immediately removes ordinary classic disease bundles while preserving special
        /// infection lifecycles such as vampirism, werewolf, and wereboar infection.
        /// Special infections derive from DiseaseEffect but deliberately expose
        /// ClassicDiseaseType == Diseases.None.
        /// </summary>
        /// <param name="reason">Diagnostic reason supplied by the caller.</param>
        /// <returns>Number of classic disease bundles removed.</returns>
        public int CureAllClassicDiseases(string reason)
        {
            int removedCount = 0;
            LiveEffectBundle[] bundles = GetDiseaseBundles();
            ...
            foreach (LiveEffectBundle bundle in bundles)
            {
                ...
                bool containsClassicDisease = false;
                bool containsSpecialInfection = false;

                foreach (IEntityEffect liveEffect in bundle.liveEffects)
                {
                    DiseaseEffect diseaseEffect = liveEffect as DiseaseEffect;
                    if (diseaseEffect == null)
                        continue;

                    if (diseaseEffect.ClassicDiseaseType == Diseases.None)
                    {
                        containsSpecialInfection = true;
                        break;
                    }

                    containsClassicDisease = true;
                }

                // Never remove a bundle containing a special infection, even if a mod has
                // placed classic and special disease effects in the same custom bundle.
                if (!containsClassicDisease || containsSpecialInfection)
                    continue;

                RemoveBundle(bundle);
                removedCount++;
                Debug.LogFormat(
                    "[EntityEffectManager] Removed classic disease bundle. reason={0} bundle={1}",
                    reason ?? string.Empty,
                    bundle.GetHashCode());
            }
        ...
        /// <summary>
        /// Rebases only ordinary classic disease daily timers to current WorldTime.
        /// Used for the first host-time application after a pure client loads a save, so
        /// that save-to-host clock distance is not treated as accumulated disease days.
        /// Normal host time synchronization does not call this method.
        /// </summary>
        /// <param name="reason">Diagnostic reason supplied by the caller.</param>
        /// <returns>Number of classic disease effects realigned.</returns>
        public int AlignClassicDiseaseTimersToCurrentTime(string reason)
        {
            int alignedCount = 0;
            LiveEffectBundle[] bundles = GetDiseaseBundles();
            ...
            foreach (LiveEffectBundle bundle in bundles)
            {
                if (bundle.liveEffects == null)
                    continue;

                foreach (IEntityEffect liveEffect in bundle.liveEffects)
                {
                    DiseaseEffect diseaseEffect = liveEffect as DiseaseEffect;
                    if (diseaseEffect == null || diseaseEffect.ClassicDiseaseType == Diseases.None)
                        continue;

                    uint previousDay = diseaseEffect.AlignLastProcessedDayToCurrentTime();
                    alignedCount++;
                    Debug.LogFormat(
                        "[EntityEffectManager] Aligned classic disease timer. reason={0} disease={1} oldDay={2}",
                        reason ?? string.Empty,
        ...
        void EnemyCastReadySpell()
        {
            ...
            EntityEffectBundle spellToCast = readySpell;

            // Play cast sound from caster audio source
            if (spellToCast.CasterEntityBehaviour)
                PlayCastSound(spellToCast.CasterEntityBehaviour, GetCastSoundID(spellToCast.Settings.ElementType));

            // Create magic sparkles effect for non-ranged casts
            if (spellToCast.Settings.TargetType != TargetTypes.SingleTargetAtRange &&
                spellToCast.Settings.TargetType != TargetTypes.AreaAtRange)
            {
                EnemyBlood sparkles = spellToCast.CasterEntityBehaviour != null ? spellToCast.CasterEntityBehaviour.GetComponent<EnemyBlood>() : null;
                CharacterController targetController = entityBehaviour != null ? entityBehaviour.transform.GetComponent<CharacterController>() : null;
                if (sparkles && targetController != null)
                {
            ...
            bool canExecuteRealCast = !networkActive || isServer;

            if (canExecuteRealCast)
            {
                if (spellToCast.Settings.TargetType == TargetTypes.CasterOnly)
                {
                    AssignBundle(spellToCast);
                }
                else
                {
                    Vector3 aimPosition;
                    Vector3 aimDirection;
                    GetEnemySpellAim(out aimPosition, out aimDirection);

                    DaggerfallMissile missile = InstantiateSpellMissile(spellToCast.Settings.ElementType);
                    if (missile)
                    {
                        missile.CustomAimPosition = aimPosition;
                        missile.CustomAimDirection = aimDirection;
                        missile.Payload = spellToCast;
                    }

                    // Remote clients get a visual-only missile. The authoritative cast above
                    // is the only one allowed to assign payload damage/effects. This prevents
                    // observer missiles from colliding with or damaging the caster/enemy copy.
                    if (isServer)
                    {
                        RpcTriggerSpellVisual(
                            spellToCast.Settings.ElementType,
                            spellToCast.Settings.TargetType,
                            aimPosition,
                            aimDirection,
                            GetEnemySpellTargetNetId(),
                            SerializeSpellData(spellToCast));
                    }
                }
            }

            // Clear ready spell
            RaiseOnCastReadySpell(spellToCast);
            ...
            // Server consumes enemy magicka for the real cast. Client-owned enemies run
            // their AI locally, so mirror the new magicka value back or the owner will
            // keep thinking this enemy can cast and will keep requesting failed spells.
            if (isServer && entityBehaviour != null && entityBehaviour.Entity != null)
                RpcSyncEnemyMagickaAfterSpell(entityBehaviour.Entity.CurrentMagicka);
        }

        private uint GetEnemySpellTargetNetId()
        {
            if (entityBehaviour == null)
                return 0;

            EnemySenses enemySenses = entityBehaviour.GetComponent<EnemySenses>();
            if (enemySenses == null || enemySenses.Target == null)
                return 0;

            NetworkIdentity targetIdentity = enemySenses.Target.GetComponent<NetworkIdentity>();
            return targetIdentity != null ? targetIdentity.netId : 0;
        }

        [ClientRpc]
        private void RpcSyncEnemyMagickaAfterSpell(int currentMagicka)
        {
            // Host/server already has the authoritative value.
            if (isServer)
                return;
        ...
        private void GetEnemySpellAim(out Vector3 aimPosition, out Vector3 aimDirection)
        {
            aimPosition = entityBehaviour != null ? entityBehaviour.transform.position : transform.position;
            aimDirection = entityBehaviour != null ? entityBehaviour.transform.forward : transform.forward;
            ...
            Vector3 targetPosition = EnemySenses.ResetPlayerPos;
            try
            {
                if (DaggerfallUnity.Settings.EnhancedCombatAI)
                    targetPosition = enemySenses.PredictNextTargetPos(25f);
                else
                    targetPosition = enemySenses.LastKnownTargetPos;
            }
            catch
            {
                targetPosition = EnemySenses.ResetPlayerPos;
            }

            if (targetPosition == EnemySenses.ResetPlayerPos && enemySenses.Target != null)
                targetPosition = enemySenses.Target.transform.position;

            if (targetPosition != EnemySenses.ResetPlayerPos)
            {
                Vector3 dir = targetPosition - aimPosition;
        ...
        [ClientRpc]
        void RpcTriggerSpellVisual(ElementTypes elementType, TargetTypes targetType, Vector3 aimPosition, Vector3 aimDirection, uint targetNetId, string spellData)
        {
            if (isServer)
                return; // host already spawned the authoritative missile locally

            EntityEffectBundle visualSpell = DeserializeSpellData(spellData);
            if (visualSpell == null)
                return;

            bool localPlayerIsTarget = false;
            try
            {
                PlayerMultiplayer localMultiplayer = PlayerMultiplayer.localPlayer;
                localPlayerIsTarget = localMultiplayer != null && targetNetId != 0 && localMultiplayer.netId == targetNetId;
            }
            catch { localPlayerIsTarget = false; }

            DaggerfallMissile missile = InstantiateSpellMissile(elementType);
            if (missile)
            {
                // Observers get a harmless visual-only missile. The owning client gets
                // a local-player-only payload missile so the spell can affect PlayerAdvanced
                // without giving the missile solid physics or letting it damage/push enemies.
                missile.VisualOnly = !localPlayerIsTarget;
                missile.LocalPlayerOnlyPayload = localPlayerIsTarget;
                missile.CustomAimPosition = aimPosition;
                missile.CustomAimDirection = aimDirection;
                missile.TargetType = targetType;
                missile.ElementType = elementType;
                missile.Payload = visualSpell;
            }
        }
        // Convert Spell Data into a String
        string SerializeSpellData(EntityEffectBundle spell)
        {
            return JsonUtility.ToJson(spell.Settings);
        }

        // Convert String Back into Spell Data
        EntityEffectBundle DeserializeSpellData(string spellData)
        {
        ...
        private void ReportLocalPlayerSpellCastVisual(EntityEffectBundle spell)
        {
            if (!IsPlayerEntity || spell == null)
                return;

            // Avoid warnings/noise in normal single-player.
            if (!NetworkClient.active && !NetworkServer.active)
                return;

            global::PlayerMultiplayer localPlayer = global::PlayerMultiplayer.GetLocalPlayerForCommand("player spell cast visual");
            if (localPlayer == null)
                return;
            ...
            if (aimDirection == Vector3.zero && GameManager.Instance != null && GameManager.Instance.PlayerObject != null)
                aimDirection = GameManager.Instance.PlayerObject.transform.forward;

            if (aimDirection == Vector3.zero)
                aimDirection = Vector3.forward;

            int castSoundID = GetCastSoundID(spell.Settings.ElementType);

            localPlayer.CmdReportPlayerSpellCastVisual(
                (int)spell.Settings.ElementType,
                (int)spell.Settings.TargetType,
        ...
        {
            // In vanilla/SP death normally ends the current game state, so wiping all
            // active bundles is safe. In MP respawn the same PlayerAdvanced object is
            // kept alive and reused after death. If we wipe bundles here, permanent
            // player effects such as Vampirism-Curse / Lycanthropy can disappear after
            // the first MP respawn, especially after a fresh game start/load.
            if (IsPlayerEntity && (NetworkServer.active || NetworkClient.active))
            {
                Debug.LogFormat("[EntityEffectManager] MP player death: preserving effect bundles for respawn on {0}.", entity != null ? entity.Name : "unknown entity");
                return;
            }
