...
namespace DaggerfallWorkshop.Game.MagicAndEffects
{
    ...
    public class EntityEffectBroker : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            // MP safety: if the broker missed normal initialization, establish the baseline here instead
            // of silently disabling all magic-round effects forever. Save/load still calls InitMagicRoundTimer()
            // normally; this only catches bad startup/order cases.
            if (lastGameMinute == 0)
            {
                lastGameMinute = gameMinute;
                magicRoundsSinceStartup = 0;
                Debug.Log($"[EntityEffectBroker] Magic round timer was uninitialized; aligned to current minute {lastGameMinute}.");
                return;
            }

            // MP host-time rollback safety. Host-authoritative time can pull a client backwards
            // after a local fast-travel/wait/vampire time jump. If lastGameMinute remains in the
            // discarded future, uint subtraction below underflows and magic rounds/effect durations
            // can freeze until host time catches up. Align to the authoritative current minute
            // without firing catch-up rounds.
            if (gameMinute < lastGameMinute)
            {
                uint oldMinute = lastGameMinute;
                lastGameMinute = gameMinute;
                SyntheticTimeIncrease = false;
                Debug.Log($"[EntityEffectBroker] Magic round timer moved back with WorldTime. oldMinute={oldMinute} newMinute={lastGameMinute} rounds={magicRoundsSinceStartup}");
                return;
            }
        ...
        /// <summary>
        /// Realigns the magic-round baseline to current WorldTime without firing catch-up rounds.
        /// Used by MP host-time sync when local time is pulled backwards. Forward syncs should
        /// not call this, because forward elapsed minutes should still tick effects naturally.
        /// </summary>
        public void AlignMagicRoundTimerToCurrentTime(string reason)
        {
            uint oldMinute = lastGameMinute;
            uint newMinute = DaggerfallUnity.Instance.WorldTime.DaggerfallDateTime.ToClassicDaggerfallTime();
            lastGameMinute = newMinute;
            if (oldMinute == 0)
                magicRoundsSinceStartup = 0;

            Debug.Log($"[EntityEffectBroker] Aligned magic round timer. reason={reason} oldMinute={oldMinute} newMinute={lastGameMinute} rounds={magicRoundsSinceStartup}");
        }
