...
// TanguyMultiplayer action sync revision: resulting-state reporting for shared dungeon actions.
    ...
    public class PlayerActivate : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (InputManager.Instance.ActionComplete(InputManager.Actions.ActivateCenterObject))
            {
                ...
                // shell/CharacterController can be on that layer. Without this, clicking a downed player
                // never reaches the revive code.
                if (TryActivateDownedMultiplayerPlayerFromRay(ray))
                    return;
                ...
                if (hitSomething)
                {
                    if (TryActivateDownedMultiplayerPlayer(hit))
                        return;
                    ...
                    {
                        int activationCountBefore = action.activationCount;
                        Vector3 originalActionPosition = action.transform.position;
                        action.Receive(this.gameObject, DaggerfallAction.TriggerTypes.Direct);

                        // Notify only after DFU accepted the Direct trigger. This preserves the
                        // normal SP path and avoids networking rejected/cooldown activations.
                        if (action.activationCount != activationCountBefore)
                        {
                            DoorsCatcher.NotifyActionActivated(
                                action,
                                originalActionPosition,
                                DaggerfallAction.TriggerTypes.Direct);
                        }
                    }
        ...
        private bool TryActivateDownedMultiplayerPlayerFromRay(Ray ray)
        {
            ...
            // PlayerMultiplayer shell can never be hit by the regular activation ray.
            RaycastHit[] hits = Physics.RaycastAll(
                ray,
                global::PlayerMultiplayer.ReviveRaycastDistance,
                ~0,
            ...
            for (int i = 0; i < hits.Length; i++)
            {
                ...
                global::PlayerMultiplayer targetPlayer = hit.transform.GetComponentInParent<global::PlayerMultiplayer>();
                if (targetPlayer == null)
                    continue;

                // Never self-revive by clicking our own local network object.
                ...
                return TryActivateDownedMultiplayerPlayer(hit, targetPlayer);
            }

            return false;
        }

        private bool TryActivateDownedMultiplayerPlayer(RaycastHit hit)
        {
            global::PlayerMultiplayer targetPlayer = hit.transform.GetComponentInParent<global::PlayerMultiplayer>();
            if (targetPlayer == null)
                return false;
        ...
        private bool TryActivateDownedMultiplayerPlayer(RaycastHit hit, global::PlayerMultiplayer targetPlayer)
        {
            if (targetPlayer == null)
                return false;
            ...
            // The local real player should not revive themselves by clicking their own network shell.
            if (targetPlayer.isLocalPlayer)
                return true;

            if (hit.distance > global::PlayerMultiplayer.ReviveInteractDistance)
            {
                DaggerfallUI.SetMidScreenText(TextManager.Instance.GetLocalizedText("youAreTooFarAway"));
                return true;
            }

            global::PlayerMultiplayer localPlayer = global::PlayerMultiplayer.GetLocalPlayerForCommand("revive-downed-player");
            if (localPlayer == null)
                return true;
