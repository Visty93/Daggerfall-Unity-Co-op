...
namespace DaggerfallWorkshop.Game
{
    ...
    public class GameManager : MonoBehaviour
    {
        ...
        public bool AreEnemiesNearby(bool resting = false, bool includingPacified = false)
        {
            ...
            foreach (DaggerfallEntityBehaviour entityBehaviour in ActiveGameObjectDatabase.GetActiveEnemyBehaviours())
            {
                if (entityBehaviour.EntityType == EntityTypes.EnemyMonster || entityBehaviour.EntityType == EntityTypes.EnemyClass)
                {
                    ...
                    if (enemySenses)
                    {
                        // Check if enemy can actively target the local player. In multiplayer, enemies
                        // target PlayerMultiplayer objects rather than GameManager.PlayerEntityBehaviour.
                        bool enemyCanSeePlayer = EnemyCanSeeLocalPlayer(enemySenses);

                        // Multiplayer dungeons/enemies are active globally and often live at scene root.
                        // enemySenses.WouldBeSpawnedInClassic is calculated by that enemy's own target/closest
                        // PlayerMultiplayer, so it can be true because another player is fighting in a dungeon.
                        // Do not let those far-away dungeon enemies block this local player's travel/rest.
                        if (networkActive && !enemyCanSeePlayer)
                        {
                            float localLimit = resting ? restingDistance : spawnDistance;
                            if ((entityBehaviour.transform.position - PlayerController.transform.position).sqrMagnitude > localLimit * localLimit)
                                continue;
                        }
        ...
        private bool EnemyCanSeeLocalPlayer(EnemySenses enemySenses)
        {
            if (enemySenses == null || enemySenses.Target == null || !enemySenses.TargetInSight)
                return false;

            // Singleplayer/default local player object.
            if (enemySenses.Target == Instance.PlayerEntityBehaviour)
                return true;

            // Multiplayer local player object. The host can have netId 0 in this project,
            // so rely on isLocalPlayer rather than targetNetId != 0 style checks.
            PlayerMultiplayer targetMultiplayer = enemySenses.Target.GetComponent<PlayerMultiplayer>();
            return targetMultiplayer != null && targetMultiplayer.isLocalPlayer;
        }
        ...
        {
            // Multiplayer safety:
            // This is a local cleanup helper used by court/prison release and other SP flows.
            // In MP, enemies are networked/root-managed and other players may still be near or
            // inside a dungeon/interior with those same enemies. Running this locally would destroy
            // the local client's copies only, desyncing the scene, and it can also delete
            // PlayerMultiplayer visuals because those contain DaggerfallEnemy-based avatar objects.
            //
            // Let the network authority/destroy/culling systems handle MP enemies instead.
            if (Mirror.NetworkClient.active || Mirror.NetworkServer.active)
            {
                Debug.Log("[ClearEnemies] Skipped local enemy cleanup because multiplayer is active.");
                return;
            }
