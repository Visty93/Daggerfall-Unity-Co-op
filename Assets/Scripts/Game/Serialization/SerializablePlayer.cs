...
namespace DaggerfallWorkshop.Game.Serialization
{
    ...
    public class SerializablePlayer : MonoBehaviour, ISerializableGameObject
    {
        ...
        // Must match PlayerEnterExit.multiplayerInteriorYOffset.
        // Used only when loading a normal SP building-interior save while MP is already active.
        // PlayerEnterExit will lower the rebuilt interior in active MP, so the saved SP player
        // position must be lowered by the same amount after SerializablePlayer restores it.
        const float multiplayerBuildingInteriorYOffset = -250f;
        ...
        public PlayerPositionData_v1 GetPlayerPositionData()
        {
            ...
            // Always store dungeon-local position and identity while inside a dungeon.
            // This lets a later MP session place the player into whichever host-authored
            // Y slot currently owns this dungeon, without restoring the old raw world Y.
            if (playerEnterExit.IsPlayerInsideDungeon && playerEnterExit.Dungeon != null)
            {
                DaggerfallDungeon savedDungeon = playerEnterExit.Dungeon;
                playerPosition.hasSavedDungeonLocalPosition = true;
                playerPosition.savedDungeonLocalPosition = transform.position - savedDungeon.transform.position;

                if (savedDungeon.Summary.LocationData.Loaded)
                {
                    playerPosition.savedDungeonRegionName = savedDungeon.Summary.RegionName;
                    playerPosition.savedDungeonLocationName = savedDungeon.Summary.LocationName;
                    playerPosition.savedDungeonMapId = savedDungeon.Summary.ID;

                    // PlayerGPS can intentionally retain an earlier exterior coordinate
                    // during an asynchronous MP dungeon handoff. Never serialize that stale
                    // coordinate as this dungeon's exit/fallback anchor. Prefer the live
                    // network dungeon's authoritative entrance, then calculate the same
                    // entrance from the saved DFLocation for local/SP dungeons.
                    if (savedDungeon.HasDungeonWorldAnchor)
                    {
                        playerPosition.worldPosX = savedDungeon.DungeonAnchorWorldX;
                        playerPosition.worldPosZ = savedDungeon.DungeonAnchorWorldZ;
                    }
                    else
                    {
                        try
                        {
                            int entranceWorldX;
                            int entranceWorldZ;
                            Vector3 entranceLocal;
                            if (DaggerfallWorkshop.StreamingWorld.TryGetDungeonEntranceWorldCoordinates(
                                savedDungeon.Summary.LocationData,
                                out entranceWorldX,
                                out entranceWorldZ,
                                out entranceLocal))
                            {
                                playerPosition.worldPosX = entranceWorldX;
                                playerPosition.worldPosZ = entranceWorldZ;
                            }
                        }
                        catch (Exception ex)
                        {
                            Debug.LogWarning($"[NetworkDungeonSave] Could not calculate authoritative saved dungeon entrance for '{savedDungeon.Summary.RegionName}/{savedDungeon.Summary.LocationName}'. Keeping PlayerGPS fallback. error={ex.Message}");
                        }
                    }
                }

                // A dungeon recovered locally in SP from an MP save must retain its MP-save
                // marker if the player saves again before reconnecting. Its object is local,
                // but its saved Y still needs normalization during the next MP conversion.
                if (savedDungeon.IsNetworkDungeonInstance || savedDungeon.IsRecoveredNetworkDungeonSave)
                {
                    playerPosition.savedInsideNetworkDungeon = true;
                    playerPosition.savedNetworkDungeonY = savedDungeon.PositionY;
                }

                Debug.Log($"[NetworkDungeonSave] Saved dungeon-local player position dungeon='{playerPosition.savedDungeonRegionName}/{playerPosition.savedDungeonLocationName}' mapId={playerPosition.savedDungeonMapId} networkOrRecovered={playerPosition.savedInsideNetworkDungeon} dungeonY={savedDungeon.transform.position.y} local={playerPosition.savedDungeonLocalPosition} world={playerPosition.position} entranceDF={playerPosition.worldPosX}/{playerPosition.worldPosZ}");
            }

            // Mark saves made inside MP building interiors. These interiors are local/non-networked,
            // but while MP is active PlayerEnterExit offsets them downward. If this save is loaded
            // in SP, the interior must be recreated with the same temporary offset so the saved
            // player/enemy positions do not fall below the building. If loaded while MP is active,
            // saved local enemies are suppressed by SaveLoadManager so host/network state remains authoritative.
            ...
            {
                playerPosition.savedInsideMultiplayerInterior = true;
                playerPosition.savedMultiplayerInteriorYOffset = -250f;

                Debug.Log($"[NetworkInteriorSave] Saved player inside MP-offset interior yOffset={playerPosition.savedMultiplayerInteriorYOffset} pos={playerPosition.position}");
            }
        ...
        public void RestoreSaveData(object dataIn)
        {
            ...
            // Restore player position. When loading a dungeon save during an active MP session,
            // PlayerEnterExit may already have forced the player outside at the dungeon entrance.
            // Do not restore the saved dungeon-local transform position afterward, or the player
            // will be placed back into the void with no dungeon object.
            if (data.playerPosition.suppressNetworkDungeonPositionRestore)
            {
                Debug.Log("[DungeonNetworkSafety] Skipping raw SerializablePlayer position restore after network dungeon conversion/recovery.");
                RestoreOrientationAndCrouch(data.playerPosition);
            }
            else if (restorePlayerPosition)
            {
        ...
        public void RestorePosition(PlayerPositionData_v1 positionData)
        {
            ...
            {
                Vector3 restoredPosition = positionData.position;

                // Loading a normal SP save inside a building while MP is active is a special case.
                // PlayerEnterExit rebuilds building interiors at the MP Y offset whenever network is active,
                // but the saved SP player transform is still at the normal SP interior height. Lower the
                // player by the same fixed offset so player and rebuilt interior remain aligned.
                if (ShouldLowerSPInteriorSavePositionForActiveMultiplayer(positionData))
                {
                    restoredPosition.y += multiplayerBuildingInteriorYOffset;
                    Debug.Log($"[NetworkInteriorSave] Lowered SP-saved building player position for active MP load by {multiplayerBuildingInteriorYOffset}. savedY={positionData.position.y} restoredY={restoredPosition.y}");
                }
            ...
            RestoreOrientationAndCrouch(positionData);
        }
        ...
        void RestoreOrientationAndCrouch(PlayerPositionData_v1 positionData)
        {
            if (positionData == null)
                return;

            // Keep view/crouch restoration even when a network dungeon conversion must
            // suppress only the old raw transform position.
            playerMouseLook.SetFacing(positionData.yaw, positionData.pitch);
            playerMotor.IsCrouching = positionData.isCrouching;
        }

        bool ShouldLowerSPInteriorSavePositionForActiveMultiplayer(PlayerPositionData_v1 positionData)
        {
            if (positionData == null)
                return false;

            // Only building interiors use the local MP -Y offset.
            if (!positionData.insideBuilding || positionData.insideDungeon)
                return false;

            // If the save was already made in an MP-offset interior, the stored player position
            // is already lowered. Do not lower it again.
            if (positionData.savedInsideMultiplayerInterior)
                return false;

            // This adjustment is only needed while loading into an active MP session, because
            // PlayerEnterExit.ShouldOffsetBuildingInteriorForMultiplayer() will rebuild the
            // interior at multiplayerInteriorYOffset when NetworkServer/NetworkClient is active.
            return Mirror.NetworkServer.active || Mirror.NetworkClient.active;
        }
