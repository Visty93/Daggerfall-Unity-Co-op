...
namespace DaggerfallWorkshop.Game.Serialization
{
    ...
    public class PlayerPositionData_v1
    {
        ...
        // Multiplayer dungeon save/recovery metadata.
        // Normal singleplayer dungeon saves leave these at defaults.
        public bool savedInsideNetworkDungeon;
        public float savedNetworkDungeonY;

        // Dungeon-local entry metadata used when an SP/MP dungeon save is loaded while
        // multiplayer is active, or when a local SP dungeon is converted on connect/host.
        // New saves store this explicitly. Older saves are migrated from position and
        // savedNetworkDungeonY (MP) or the normal Y=0 dungeon root (SP).
        public bool hasSavedDungeonLocalPosition;
        public Vector3 savedDungeonLocalPosition;
        public string savedDungeonRegionName;
        public string savedDungeonLocationName;
        public int savedDungeonMapId;

        // Multiplayer building-interior save/recovery metadata.
        // These interiors are not networked, but while MP is active they are deliberately
        // spawned below the exterior world by PlayerEnterExit. This flag lets SP load
        // recreate that same temporary offset so the saved player/enemy positions line up.
        public bool savedInsideMultiplayerInterior;
        public float savedMultiplayerInteriorYOffset;

        // Runtime-only load safety flag. This is set after deserializing when a dungeon save
        // must be restored to exterior during an active MP session. It prevents
        // SerializablePlayer.RestoreSaveData() from putting the player back at the saved
        // dungeon-local transform position after PlayerEnterExit has already kicked them out.
        public bool suppressNetworkDungeonPositionRestore;
