    ...
    public class DaggerfallAction : MonoBehaviour
    {
        ...
        public void UserInputHandler(DaggerfallInputMessageBox sender, string userInput)
        {
            ...
            for (int i = 0; i < type12_answers.Length; i++)
            {
                ...
                {
                    DaggerfallAction nextAction = NextObject != null
                        ? NextObject.GetComponent<DaggerfallAction>()
                        : null;
                    Vector3 nextOriginalPosition = nextAction != null
                        ? nextAction.transform.position
                        : Vector3.zero;

                    if (!DoorsCatcher.TryBeginSharedTextAnswer(this, nextAction))
                        return;
                    ...
                    if (nextAction != null)
                    {
                        DoorsCatcher.NotifySharedTextAnswer(
                            this,
                            nextAction,
        ...
        {
            // Another player already supplied the correct answer and the following world
            // mechanism was replayed locally. Do not allow a later duplicate answer to
            // toggle that shared trapdoor/barrier back again.
            if (DoorsCatcher.IsTextAnswerCompleted(thisAction))
                return;
