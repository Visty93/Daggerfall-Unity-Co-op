...
public void OverrideDoorOwner(Transform newOwner)
{
    if (newOwner != null)
    {
        doorOwner = newOwner;
        Debug.Log($"[InteriorNet] 🔁 Door owner for {name} overridden with {newOwner.name}");
    }
    else
    {
        Debug.LogWarning("[InteriorNet] ⚠ Tried to override doorOwner with null");
    }
}



public void SetDoorOwner(Transform owner)
{
    doorOwner = owner;
}
	...
	public bool DoLayoutMultiplayer(Transform doorOwner, StaticDoor door, ClimateBases climateBase, PlayerGPS.DiscoveredBuilding buildingData, DFLocation forcedLocation)
{
    if (dfUnity == null)
        dfUnity = DaggerfallUnity.Instance;
    ...
    // Use forced location instead of PlayerGPS.CurrentLocation
    DFBlock[] blocks = RMBLayout.GetLocationBuildingData(forcedLocation);
    ...
    if (!foundBlock || this.blockData.Type != DFBlock.BlockTypes.Rmb)
        throw new Exception($"[Multiplayer] Could not load RMB block index {door.blockIndex}");

    // Get record data
if (door.recordIndex >= blockData.RmbBlock.SubRecords.Length)
{
    Debug.LogError($"[InteriorNet] ❌ Invalid recordIndex: {door.recordIndex} >= {blockData.RmbBlock.SubRecords.Length} for blockIndex: {door.blockIndex}");
    return false;
}

    recordData = blockData.RmbBlock.SubRecords[door.recordIndex];
    if (recordData.Interior.Header.Num3dObjectRecords == 0)
        throw new Exception($"[Multiplayer] No interior 3D models found for record index {door.recordIndex}");

    // Continue interior layout
    AddModels(buildingData);
    AddFlats(buildingData);
    AddPeople(buildingData);
    AddActionDoors();
