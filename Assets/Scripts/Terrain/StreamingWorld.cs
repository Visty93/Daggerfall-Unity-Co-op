...
namespace DaggerfallWorkshop
{
    ...
    public class StreamingWorld : MonoBehaviour
    {
       ...
       public DaggerfallLocation currentPlayerLocationObject;
        ...
        /// <summary>
        /// True while a teleport/map-pixel change is still rebuilding terrain or
        /// refreshing exterior location objects. This read-only status is used by
        /// multiplayer party travel to keep the HUD black until arrival is ready.
        /// It does not change normal streaming behaviour.
        /// </summary>
        public bool IsWorldUpdateRunning
        {
            get { return init || terrainUpdateRunning || updateLocations; }
        }
        ...
        /// <summary>
        /// Teleport to specific world coordinates while preserving an exterior Unity Y height.
        /// RepositionPlayer() still clamps the result up to terrain height when this value is
        /// stale or below ground, but keeps it when it is above terrain (stairs, platforms, roofs).
        /// </summary>
        public void TeleportToWorldCoordinates(int worldPosX, int worldPosZ, float exteriorWorldY)
        {
            // Convert world coordinates to map pixel and find origin world position of tile
            DFPosition mapPixel = MapsFile.WorldCoordToMapPixel(worldPosX, worldPosZ);
            DFPosition originWorldPos = MapsFile.MapPixelToWorldCoord(mapPixel.X, mapPixel.Y);

            // Horizontal values are terrain-tile offsets. Exterior Y is already shared in
            // Unity world space by all peers and is therefore passed through unchanged.
            Vector3 offset = new Vector3(
                (worldPosX - originWorldPos.X) / SceneMapRatio,
                exteriorWorldY,
                (worldPosZ - originWorldPos.Y) / SceneMapRatio);
        ...
        /// <summary>
        /// Calculates the same exterior dungeon-entrance world coordinates used by
        /// RepositionMethods.DungeonEntrance, without moving the real StreamingWorld or player.
        /// This is used by multiplayer quest teleports so TeleportPc can publish the
        /// exact same DF X/Z as a normal manual dungeon entrance before network dungeon
        /// generation/imported enemies run.
        /// </summary>
        public static bool TryGetDungeonEntranceWorldCoordinates(DFLocation location, out int worldX, out int worldZ, out Vector3 localEntrancePosition)
        {
            worldX = 0;
            worldZ = 0;
            localEntrancePosition = Vector3.zero;

            if (!location.Loaded)
                return false;

            GameObject tempLocationObject = null;
            ...
            try
            {
                ...
                tempLocationObject = new GameObject($"TeleportPcDungeonEntranceProbe [{location.RegionName}/{location.Name}]");
                tempLocationObject.hideFlags = HideFlags.HideAndDontSave;
                tempLocationObject.transform.position = new Vector3(0, -20000f, 0);
                tempLocationObject.transform.rotation = Quaternion.identity;

                DaggerfallLocation dfLocation = tempLocationObject.AddComponent<DaggerfallLocation>();
                dfLocation.SetLocation(location, false);

                BuildingDirectory buildingDirectory = tempLocationObject.AddComponent<BuildingDirectory>();
                DFBlock[] blocks;
                ...
                // This temporary probe only needs enough block layout to create static doors.
                // Use the current PlayerGPS nature texture set for billboard batches;
                // the exact nature archive does not affect dungeon entrance door placement.
                int natureArchive = ClimateSwaps.GetNatureArchive(
                    GameManager.Instance.PlayerGPS.ClimateSettings.NatureSet,
                    dfUnity.WorldTime.Now.SeasonValue);
                DaggerfallBillboardBatch natureBillboardBatch = GameObjectHelper.CreateBillboardBatchGameObject(natureArchive, tempLocationObject.transform);
                DaggerfallBillboardBatch lightsBillboardBatch = GameObjectHelper.CreateBillboardBatchGameObject(TextureReader.LightsTextureArchive, tempLocationObject.transform);
                DaggerfallBillboardBatch animalsBillboardBatch = GameObjectHelper.CreateBillboardBatchGameObject(TextureReader.AnimalsTextureArchive, tempLocationObject.transform);
                ...
                for (int y = 0; y < height; ++y)
                {
                    for (int x = 0; x < width; ++x, ++blockIndex)
                    {
                        ...
                        GameObject blockObject = GameObjectHelper.CreateRMBBlockGameObject(
                            blocks[blockIndex],
                            x,
                            y,
                        ...
                        if (blockObject != null)
                        {
                            blockObject.hideFlags = HideFlags.HideAndDontSave;
                            blockObject.transform.parent = tempLocationObject.transform;
                            blockObject.transform.localPosition = blockOrigin;
                        }
                    }
                }

                DaggerfallStaticDoors[] doorCollections = tempLocationObject.GetComponentsInChildren<DaggerfallStaticDoors>(true);
                ...
                foreach (DaggerfallStaticDoors collection in doorCollections)
                {
                    ...
                    for (int i = 0; i < collection.Doors.Length; i++)
                    {
                        ...
                        if (doorPosition.y < lowestHeight)
                        {
                            lowestHeight = doorPosition.y;
                ...
                if (foundCollection == null || foundIndex < 0)
                    return false;

                float playerRadius = 0.5f;
                if (GameManager.Instance != null && GameManager.Instance.PlayerController != null)
                    playerRadius = GameManager.Instance.PlayerController.radius;

                localEntrancePosition = foundCollection.GetDoorPosition(foundIndex);
                localEntrancePosition += foundDoorNormal * (playerRadius + 0.1f);
                ...
                DFPosition mapPixelOrigin = MapsFile.MapPixelToWorldCoord(mapPixel.X, mapPixel.Y);

                worldX = mapPixelOrigin.X + Mathf.RoundToInt(localEntrancePosition.x * SceneMapRatio);
                worldZ = mapPixelOrigin.Y + Mathf.RoundToInt(localEntrancePosition.z * SceneMapRatio);

                Debug.Log($"[TeleportPcMP][ExactEntrance] Calculated dungeon entrance anchor for '{location.RegionName}/{location.Name}' mapPixel={mapPixel.X}/{mapPixel.Y} local={localEntrancePosition} world={worldX}/{worldZ}");
                return true;
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"[TeleportPcMP][ExactEntrance] Failed to calculate dungeon entrance anchor for '{location.RegionName}/{location.Name}'. error={ex}");
                return false;
            }
            finally
            {
                if (tempLocationObject != null)
                {
                    if (Application.isPlaying)
                        UnityEngine.Object.Destroy(tempLocationObject);
                    else
                        UnityEngine.Object.DestroyImmediate(tempLocationObject);
                }
            }
        }
