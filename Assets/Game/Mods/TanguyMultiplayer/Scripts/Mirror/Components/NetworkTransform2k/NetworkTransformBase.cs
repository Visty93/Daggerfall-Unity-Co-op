...
namespace Mirror
{
    public abstract class NetworkTransformBase : NetworkBehaviour
    {
        ...
        {
			 // DEBUG: see if we are even sending, and print the interval product
    //Debug.Log($"[NTB OnClientToServerSync] t={Time.time:F2} onlySyncOnChange={onlySyncOnChange} sendInterval={sendInterval:F2} bufMul={bufferResetMultiplier} product={sendInterval*bufferResetMultiplier:F2}");
        ...
        void UpdateServer()
        {
            ...
            //
            // Important listen-host case:
            // In this project, enemies can be client-authoritative while the listen-host/server
            // copy intentionally does NOT simulate EnemyMotor for remote-owned enemies.
            // Therefore the host/server transform must consume the remote owner's snapshots.
            //
            // The old condition was only `clientAuthority && !hasAuthority`. After a disconnect
            // handoff, the host client can still report/local-cache hasAuthority even though the
            // server-side real owner is a remote connection. In that case the serverBuffer was
            // receiving Client2 poses, but the host/server transform never applied them.
            bool ownedByRemoteClient = false;
            if (isServer && clientAuthority && connectionToClient != null)
            {
                NetworkConnectionToClient hostConnection = NetworkServer.localConnection as NetworkConnectionToClient;
                ownedByRemoteClient = hostConnection == null || connectionToClient != hostConnection;
            }

            if (clientAuthority && (!hasAuthority || ownedByRemoteClient))
            {
