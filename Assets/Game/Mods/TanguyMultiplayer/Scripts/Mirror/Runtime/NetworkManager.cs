...
{
    Debug.Log($"[NetworkManager] Client disconnected: {conn.connectionId}");

    // Safely copy client-owned objects to a list to avoid collection modification errors
    var ownedObjects = conn.clientOwnedObjects != null ? new List<NetworkIdentity>(conn.clientOwnedObjects) : null;

    if (ownedObjects != null)
    {
        foreach (NetworkIdentity ownedObject in ownedObjects)
        {
            if (ownedObject != null)
            {
                // Check if the object has the "Enemy" tag to determine if it should be kept
                if (ownedObject.gameObject.CompareTag("Enemy"))
                {
                    Debug.Log($"[NetworkManager] Keeping enemy object: {ownedObject.name} (NetID: {ownedObject.netId})");

                    // Do NOT force-assign disconnected client-owned enemies to the listen host.
                    // DynamicEnemyAuthority is responsible for selecting the next nearby player.
                    // The host reclaim path can leave the host-local copy with stale hasAuthority,
                    // which then ignores snap/warmup/NetworkTransform updates and freezes the
                    // server-side enemy position.
                    if (NetworkServer.active)
                    {
                        Debug.Log($"[NetworkManager] Removing disconnected client authority only for enemy: {ownedObject.name} (NetID: {ownedObject.netId}). DynamicEnemyAuthority will pick the next owner.");
                        ownedObject.RemoveClientAuthority();
                    }

                    // Clear target to prevent targeting a destroyed object using reflection
                    var enemySenses = ownedObject.GetComponent("EnemySenses");
                    if (enemySenses != null)
                    {
                        var targetProperty = enemySenses.GetType().GetProperty("Target");
                        if (targetProperty != null)
                        {
                            var targetValue = targetProperty.GetValue(enemySenses);
                            if (targetValue != null && targetValue.GetType().Name == "PlayerMultiplayer")
                            {
                                Debug.Log($"[NetworkManager] Clearing enemy target for: {ownedObject.name} (NetID: {ownedObject.netId})");

                                // Invoke SetTarget(null) using reflection
                                var setTargetMethod = enemySenses.GetType().GetMethod("SetTarget");
                                if (setTargetMethod != null)
                                {
                                    setTargetMethod.Invoke(enemySenses, new object[] { null });
                                    Debug.Log($"[NetworkManager] Successfully cleared enemy target for: {ownedObject.name}");
                                }
                                else
                                {
                                    Debug.LogWarning($"[NetworkManager] Failed to find SetTarget method on: {ownedObject.name}");
                                }
                            }
                        }
                    }

                    // The enemy is already a spawned server object. Do not call NetworkServer.Spawn()
                    // again here; just keep it alive and let DynamicEnemyAuthority reassign owner.
                }
                else
                {
                    Debug.Log($"[NetworkManager] Destroying non-enemy object: {ownedObject.name} (NetID: {ownedObject.netId})");
                    NetworkServer.Destroy(ownedObject.gameObject);
                }
            }
        }
    }

    // Destroy the player object for the disconnected client
    if (conn.identity != null)
    {
        Debug.Log($"[NetworkManager] Destroying player object for connection: {conn.connectionId}");
        NetworkServer.DestroyPlayerForConnection(conn);
    }

    Debug.Log($"[NetworkManager] Custom disconnect logic executed for client: {conn.connectionId}");
}
