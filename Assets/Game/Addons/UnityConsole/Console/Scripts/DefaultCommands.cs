    ...
    public class DefaultCommands : MonoBehaviour
    {
        ...
        private static class TeleportToQuestMarker
        {
            ...
            public static string Execute(params string[] args)
            {
                ...
                if (playerEnterExit == null || playerObject == null)
                    return error;
                ...
                if (playerEnterExit.IsPlayerInsideDungeon)
                {
                    DaggerfallDungeon closestDungeon = FindClosestGeneratedDungeon(playerObject.transform.position);
                    if (closestDungeon == null)
                        return error;

                    // In MP several dungeons can exist at once. Use the map ID owned by the
                    // physically closest dungeon rather than potentially stale global PlayerGPS context.
                    if (!QuestMachine.Instance.GetLocationQuestMarker(
                        SiteTypes.Dungeon, closestDungeon.Summary.ID, out spawnMarker))
                    {
                        return error;
                    }
                    ...
                    Vector3 dungeonBlockPosition = new Vector3(
                        spawnMarker.dungeonX * RDBLayout.RDBSide,
                        0,
                        spawnMarker.dungeonZ * RDBLayout.RDBSide);
                    Vector3 markerLocalPosition = dungeonBlockPosition + spawnMarker.flatPosition;

                    // Quest marker coordinates are dungeon-local. Convert them through the selected
                    // live dungeon root so MP Y slots (-500, -800, -1100, ...) are respected.
                    playerObject.transform.position = closestDungeon.transform.TransformPoint(markerLocalPosition);
                }
                else if (playerEnterExit.IsPlayerInsideBuilding)
                {
                    ...
                    Vector3 markerPos;
                    if (playerEnterExit.Interior.FindClosestMarker(
                        out markerPos,
                        (DaggerfallInterior.InteriorMarkerTypes)spawnMarker.markerType,
                        playerObject.transform.position))
                    {
                        playerObject.transform.position = markerPos;
                    }
                }
                else
                {
            ...
            private static DaggerfallDungeon FindClosestGeneratedDungeon(Vector3 playerPosition)
            {
                DaggerfallDungeon[] dungeons = UnityEngine.Object.FindObjectsOfType<DaggerfallDungeon>();
                DaggerfallDungeon closestDungeon = null;
                float closestYDistance = float.MaxValue;
                float closestWorldDistance = float.MaxValue;

                foreach (DaggerfallDungeon dungeon in dungeons)
                {
                    if (dungeon == null || !dungeon.gameObject.activeInHierarchy || !dungeon.isSet)
                        continue;

                    if (!dungeon.Summary.LocationData.Loaded)
                        continue;

                    float yDistance = Mathf.Abs(playerPosition.y - dungeon.transform.position.y);
                    float worldDistance = (playerPosition - dungeon.transform.position).sqrMagnitude;

                    // Y is the primary discriminator because MP dungeons are vertically stacked.
                    // Full world distance only makes an exact-Y tie deterministic.
                    if (yDistance < closestYDistance ||
                        (Mathf.Approximately(yDistance, closestYDistance) && worldDistance < closestWorldDistance))
                    {
